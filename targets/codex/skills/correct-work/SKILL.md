...
Resolve one exact current Schema-6 Root/Evidence/Review chain from this task. Proceed only when the unique review tip says `next_action: correct` and names bounded Findings. Unsupported state, ambiguity, changed authority, new scope, risk, or acceptance requires a human-approved Schema-6 replan.
...
Finish normally without creating Evidence. A fresh `$review-work` creates delta Evidence through the generic harness boundary. Do not merge, push, publish, deploy, install, or create external effects.
