...
Recompute the baseline and stop on drift. Call `workflow_closeout` with exact artifacts and observed Check evidence. Return its Schema-5 `delivery-evidence` byte-for-byte with the `de-*` ID and any attach warning. This command grants no implementation, acceptance, learning, merge, push, publish, or deployment authority.
