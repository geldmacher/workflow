...
Collect exact Schema-6 Root/Evidence/Review bytes from this task only. Reject every unsupported artifact schema. Do not restore authority from `workflow_artifact_context`, hook state, cache, or another task. `workflow_status` must receive the explicit complete chain. Missing Evidence means fresh Review is next, not invented status.
