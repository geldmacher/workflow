...
Before mutation, capture repository identity, status, relevant fingerprints, and the complete baseline needed to distinguish pre-existing work. Preserve unrelated changes. Stay inside allowed roots, protected paths, dependency rules, external-effect limits, and delivery boundaries. Stop for new scope or authority.
...
Portable clients do not standardize native lifecycle receipts. Unless the client supplies compatible protected host receipts, MCP must downgrade machine claims and expose one current-delivery Problem. Do not loop or fabricate proof; report the provisional boundary and continue to fresh human review.
