...
Use one exact current Schema-6 Root and exact predecessor artifacts from the conversation. In a fresh task, require explicit attachment of the Root and every referenced artifact. Before repository inspection, invoke `${PLUGIN_ROOT}/dist/manual-workflow.mjs validate-plan` on the current native Plan or attached Root and stop unless it returns the expected Root ID. The builder extracts the single embedded Root from a native container; reuse the unchanged input for Review. Cache, MCP state, hook state, IDs, and harness self-assertion are not authority. Invalid, stale, conflicting, or ambiguous bytes are rejected.
