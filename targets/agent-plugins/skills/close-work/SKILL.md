...
Recompute the state and stop on drift. Call `workflow_closeout` with the exact Root, lineage artifacts when applicable, changed paths, direct Check observations, and repository snapshot. Report `Quick decision`, complete human `Details`, then the authoritative `Agent and machine contract`; claim completion only from the returned exact Evidence. This skill grants no implementation, correction, acceptance, learning, merge, push, publication, deployment, or installation authority.
