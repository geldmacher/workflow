...
Collect the exact current Schema-6 artifacts for one `wp-*` Root. Reject every unsupported artifact schema. Use cache only for non-authoritative transport enrichment. Set `presentation_locale` to `de` only when the human's active request is German, otherwise `en`, then invoke `${PLUGIN_ROOT}/dist/manual-workflow.mjs status` with the exact Root and complete chain.

Render `human_output` once and decorate only `snapshot.next_action` through the fixed portable mapping. Do not independently assess or add another action. Missing Evidence routes to fresh Review, not invented status. Do not infer commands, tools, models, harness strategy, Controller execution, or native host enforcement.
