...
Treat this invocation as authority only for the named correction. Capture the baseline, preserve unrelated work, and stay inside the Root authority envelope. Run correction Checks plus every inherited required Root Check not already passed; equivalent probes may run once, but each Check keeps honest Evidence.
Treat this invocation as authority only for the named correction. Capture the baseline, preserve unrelated work, and stay inside the Root authority envelope. Run correction Checks plus every inherited required Root Check not already passed as exact standalone planned commands; equivalent probes may run once, but each Check keeps honest Evidence.
...
If the portable client cannot supply protected host receipts, preserve the returned downgrade and route it to fresh review instead of fabricating verified correction Evidence.
