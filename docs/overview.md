...
For required machine-verifiable Checks, the fresh reviewer directly runs or inspects the planned method in its planned directory. Cursor's native task receipt transports exact Root/chain bytes but does not upgrade Check proof. Optional proof receipts may enrich Manual evidence; missing or stale proof is shown as a concrete gap instead of being presented as verified. High-risk work selects a deeper audit route and keeps unresolved uncertainty visible for the human decision.
