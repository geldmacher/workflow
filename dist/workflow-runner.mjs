...
function acquireLock(path) {
  ...
  try {
    ...
    writeFileSync(descriptor, `${JSON.stringify({ pid: process.pid, at: (/* @__PURE__ */ new Date()).toISOString() })}
`);
    return descriptor;
  } catch (error) {
...
var RunStore = class {
  ...
  runDirectory(runId2) {
    return join(this.root, "runs", runId2);
  }
  runPath(runId2) {
    return join(this.runDirectory(runId2), "run.json");
  }
  eventPath(runId2) {
    return join(this.runDirectory(runId2), "events.jsonl");
  }
  create(input) {
    ...
    try {
      ...
      const runId2 = input.run_id ?? `run-${(/* @__PURE__ */ new Date()).toISOString().replace(/[-:.TZ]/g, "")}-${randomUUID().slice(0, 8)}`;
      if (existsSync(this.runPath(runId2))) throw new Error(`run already exists: ${runId2}`);
      const run = {
      ...
      atomicJson(this.runPath(runId2), run);
      this.appendEvent(runId2, "run-created", { requested_profile: run.requested_profile });
      return run;
    } finally {
      releaseLock(lockPath, descriptor);
    }
  }
  createFromPreparation(preparationStore2, options, input) {
    const lockPath = join(this.root, ".create.lock");
    const descriptor = acquireLock(lockPath);
    try {
      ...
      if (existing) {
        ...
        const preparation2 = preparationStore2.get(options.preparationId);
        if (preparation2.status !== "consumed") preparationStore2.consume(preparation2.preparation_id, preparation2.revision, existing.run_id, options.idempotencyKey);
        return { run: existing, preparation: preparationStore2.get(options.preparationId), duplicate: true };
      }
      const preparation = preparationStore2.get(options.preparationId);
      ...
      atomicJson(this.runPath(runId2), run);
      this.appendEvent(runId2, "run-created-from-preparation", { requested_profile: run.requested_profile, preparation_id: preparation.preparation_id });
      const consumed = preparationStore2.consume(preparation.preparation_id, preparation.revision, runId2, options.idempotencyKey);
  ...
  get(runId2) {
    if (!existsSync(this.runPath(runId2))) throw new Error(`unknown run ${runId2}`);
    const run = JSON.parse(readFileSync(this.runPath(runId2), "utf8"));
  ...
  update(runId2, expectedRevision, idempotencyKey, mutator, eventType = "run-updated") {
    const lockPath = join(this.runDirectory(runId2), ".lock");
    const descriptor = acquireLock(lockPath);
    try {
      const run = this.get(runId2);
      assertCompatibleRun(run);
      if (idempotencyKey && run.idempotency?.[idempotencyKey]) return run;
      if (expectedRevision !== void 0 && expectedRevision !== run.revision) throw new Error(`revision conflict: expected ${expectedRevision}, current ${run.revision}`);
      ...
      atomicJson(this.runPath(runId2), updated);
      this.appendEvent(runId2, eventType, { revision: updated.revision });
      return updated;
    } finally {
      releaseLock(lockPath, descriptor);
    }
...
`, { mode: 384 });
  ...
  events(runId2, after = 0) {
    ...
    return readFileSync(this.eventPath(runId2), "utf8").trim().split("\n").filter(Boolean).map((line) => JSON.parse(line)).slice(after);
  }
  list() {
...
var PreparationStore = class {
  ...
  preparationDirectory(preparationId2) {
    return join(this.root, "preparations", preparationId2);
  }
  preparationPath(preparationId2) {
    return join(this.preparationDirectory(preparationId2), "preparation.json");
  }
  eventPath(preparationId2) {
    return join(this.preparationDirectory(preparationId2), "events.jsonl");
  }
  create(input) {
    ...
    try {
      ...
      const preparationId2 = input.preparation_id ?? `prep-${(/* @__PURE__ */ new Date()).toISOString().replace(/[-:.TZ]/g, "")}-${randomUUID().slice(0, 8)}`;
      if (existsSync(this.preparationPath(preparationId2))) throw new Error(`preparation already exists: ${preparationId2}`);
      const preparation = {
        ...structuredClone(input),
        ...preparationProtocolFields(),
        preparation_id: preparationId2,
        revision: 0,
        created_at: (/* @__PURE__ */ new Date()).toISOString(),
        updated_at: (/* @__PURE__ */ new Date()).toISOString(),
        controller_pid: process.pid,
      ...
      atomicJson(this.preparationPath(preparationId2), preparation);
      this.appendEvent(preparationId2, "preparation-created", { source_kind: preparation.source_kind, requested_profile: preparation.requested_profile });
      return preparation;
    } finally {
      releaseLock(lockPath, descriptor);
    }
  ...
  get(preparationId2) {
    if (!existsSync(this.preparationPath(preparationId2))) throw new Error(`unknown preparation ${preparationId2}`);
    const preparation = JSON.parse(readFileSync(this.preparationPath(preparationId2), "utf8"));
  ...
  update(preparationId2, expectedRevision, idempotencyKey, mutator, eventType = "preparation-updated") {
    const lockPath = join(this.preparationDirectory(preparationId2), ".lock");
    const descriptor = acquireLock(lockPath);
    try {
      const preparation = this.get(preparationId2);
      assertCompatiblePreparation(preparation);
      if (idempotencyKey && preparation.idempotency?.[idempotencyKey]) return preparation;
      if (expectedRevision !== void 0 && expectedRevision !== preparation.revision) throw new Error(`preparation revision conflict: expected ${expectedRevision}, current ${preparation.revision}`);
      ...
      if (idempotencyKey) updated.idempotency[idempotencyKey] = updated.revision;
      atomicJson(this.preparationPath(preparationId2), updated);
      this.appendEvent(preparationId2, eventType, { revision: updated.revision });
      return updated;
    } finally {
      releaseLock(lockPath, descriptor);
    }
  }
  consume(preparationId2, expectedRevision, runId2, idempotencyKey) {
    return this.update(preparationId2, expectedRevision, idempotencyKey, (draft) => ({
      ...draft,
      status: "consumed",
      consumed_by_run_id: runId2,
      consumed_at: (/* @__PURE__ */ new Date()).toISOString(),
      runner_pid: null
    }), "preparation-consumed");
  }
  controlUpdate(preparationId2, expectedRevision, idempotencyKey, mutator, eventType = "preparation-controlled") {
    const repositoryLockPath = join(this.root, ".create.lock");
    const descriptor = acquireLock(repositoryLockPath);
    try {
      const before = this.get(preparationId2);
      ...
      return {
        preparation: this.update(preparationId2, before.revision, idempotencyKey, mutator, eventType),
        duplicate: false
      };
    } finally {
      releaseLock(repositoryLockPath, descriptor);
    }
  }
  appendEvent(preparationId2, type, payload = {}) {
    mkdirSync(this.preparationDirectory(preparationId2), { recursive: true, mode: 448 });
    appendFileSync(this.eventPath(preparationId2), `${JSON.stringify({ id: randomUUID(), at: (/* @__PURE__ */ new Date()).toISOString(), type, payload })}
`, { mode: 384 });
  }
  events(preparationId2, after = 0) {
    if (!existsSync(this.eventPath(preparationId2))) return [];
    return readFileSync(this.eventPath(preparationId2), "utf8").trim().split("\n").filter(Boolean).map((line) => JSON.parse(line)).slice(after);
  }
  list() {
    const root = join(this.root, "preparations");
    if (!existsSync(root)) return [];
    return readdirSync(root, { withFileTypes: true }).filter((entry) => entry.isDirectory() && existsSync(this.preparationPath(entry.name))).map((entry) => this.get(entry.name));
  }
  active() {
...
var scriptDirectory = dirname2(fileURLToPath(import.meta.url));
var defaultRoot = dirname2(scriptDirectory);
...
function evidenceData(artifact) {
  ...
  const snapshot2 = tableRows(artifact.sections.get("Repository snapshot") ?? "", tables.snapshot)[0];
...
function validateEvidence(parsed, sections, failures) {
  ...
  if (snapshot2.rows.length === 1) {
    const row = snapshot2.rows[0];
...
function validateCompactReview(parsed, sections, failures) {
  ...
  let minimumRoute = actualAuditors.has("risk-auditor") || findings.rows.some((row) => /^(?:high|critical)$/.test(row.Severity)) ? "full" : actualAuditors.has("delivery-auditor") ? "targeted" : "inline";
...
function buildArtifact(text, root, options = {}) {
  ...
  if (!existsSync2(path)) return { failures: [`unsupported artifact type: ${parsed.fields.artifact}`], diagnostics, normalizations, parsed };
...
function inspectCompactArtifactSet(entries, root = defaultRoot) {
  ...
  for (const [rootId, reviews] of reviewsByRoot) {
    ...
    for (let index = 2; index < ordered.length; index += 1) {
      ...
      const states2 = window.map((review) => progressState(review, artifacts));
      for (const key of states2[2].keys()) {
        if (!states2[0].has(key) || !states2[1].has(key)) continue;
        const progressed = measurableProgress(states2[0].get(key), states2[1].get(key)) || measurableProgress(states2[1].get(key), states2[2].get(key));
        if (!progressed) {
...
import { existsSync as existsSync6, mkdirSync as mkdirSync5, readFileSync as readFileSync4, renameSync as renameSync3, writeFileSync as writeFileSync4 } from "node:fs";
import { platform as osPlatform, release as osRelease } from "node:os";
import { dirname as dirname6, join as join6 } from "node:path";

// src/controller/sandbox.mjs
import { existsSync as existsSync3, mkdtempSync, mkdirSync as mkdirSync2, rmSync, writeFileSync as writeFileSync2 } from "node:fs";
import { tmpdir } from "node:os";
import { dirname as dirname3, join as join3, resolve as resolve3 } from "node:path";
import { spawnSync } from "node:child_process";
var sandboxExecutable = "/usr/bin/sandbox-exec";
function escapeProfilePath(path) {
  return resolve3(path).replaceAll("\\", "\\\\").replaceAll('"', '\\"');
}
function buildSandboxProfile({ writablePaths = [], deniedPaths = [], deniedReadPaths = [], network = false }) {
  const writable = [...new Set(writablePaths.map((path) => resolve3(path)))];
  const denied = [...new Set(deniedPaths.map((path) => resolve3(path)))];
  const deniedRead = [...new Set(deniedReadPaths.map((path) => resolve3(path)))];
  const clauses = [
    "(version 1)",
    "(deny default)",
    "(allow process*)",
...
function probeSandboxBoundary() {
  if (process.platform !== "darwin" || !existsSync3(sandboxExecutable)) return {
    available: false,
    verified: false,
    reason: "sandbox-exec-unavailable"
  };
  const root = mkdtempSync(join3(tmpdir(), "workflow-sandbox-probe-"));
  const allowed = join3(root, "allowed");
  const denied = join3(root, "denied");
  mkdirSync2(allowed);
  mkdirSync2(denied);
  const profile = join3(root, "profile.sb");
  writeFileSync2(profile, buildSandboxProfile({ writablePaths: [allowed] }), { mode: 384 });
  const script = [
  ...
  const result = spawnSync(sandboxExecutable, ["-f", profile, process.execPath, "-e", script, join3(allowed, "ok"), join3(denied, "bad")], {
    encoding: "utf8",
    timeout: 1e4
  });
...
function runSandboxedProcess({ entrypoint, payload, writablePaths = [], deniedPaths = [], deniedReadPaths = [], network = false, timeoutMs = 3e5, environment = {}, inheritEnvironment = true }) {
  if (process.platform !== "darwin" || !existsSync3(sandboxExecutable)) throw new Error("hard sandbox unavailable; worker execution denied");
  const runRoot = writablePaths[0] ?? dirname3(resolve3(entrypoint));
  mkdirSync2(runRoot, { recursive: true, mode: 448 });
  const profilePath = join3(runRoot, `sandbox-${process.pid}-${Date.now()}.sb`);
  writeFileSync2(profilePath, buildSandboxProfile({ writablePaths, deniedPaths, deniedReadPaths, network }), { mode: 384 });
  const result = spawnSync(sandboxExecutable, ["-f", profilePath, process.execPath, resolve3(entrypoint)], {
...
import { cpSync, existsSync as existsSync4, lstatSync, mkdirSync as mkdirSync3, readFileSync as readFileSync3, readdirSync as readdirSync2, renameSync as renameSync2, rmSync as rmSync2, writeFileSync as writeFileSync3 } from "node:fs";
import { homedir } from "node:os";
import { basename as basename2, dirname as dirname4, join as join4, relative, resolve as resolve4, sep } from "node:path";
var WORKER_RUNTIME_SCHEMA = 1;
function sha2562(value) {
  return createHash3("sha256").update(value).digest("hex");
}
function sha256File(path) {
  return sha2562(readFileSync3(path));
}
function currentPlatform() {
  return `${process.platform}-${process.arch}`;
}
function defaultRuntimeRoot() {
  return join4(homedir(), ".cursor", "geldmacher-workflow", "runtime");
}
function workerRuntimeDirectory({ pluginVersion, sdkVersion: sdkVersion2, platform = currentPlatform(), runtimeRoot = defaultRuntimeRoot() }) {
  return join4(resolve4(runtimeRoot), pluginVersion, sdkVersion2, platform);
}
...
function lockInventoryHash(lockPath) {
  ...
  return sha2562(JSON.stringify(inventoryFromLock(lock)));
}
function ignoredPluginPath(path, pluginRoot2) {
  const item = relative(pluginRoot2, path);
...
function hashPluginTree(pluginRoot2) {
  const root = resolve4(pluginRoot2);
  const entries = [];
  function visit(path) {
    if (ignoredPluginPath(path, root)) return;
    const stat = lstatSync(path);
    if (stat.isSymbolicLink()) {
      entries.push(`${relative(root, path)}\0symlink\0${readFileSync3(path, "utf8")}`);
      return;
    }
    if (stat.isDirectory()) {
      for (const child of readdirSync2(path).sort()) visit(join4(path, child));
      return;
    }
    if (stat.isFile()) entries.push(`${relative(root, path)}\0${stat.mode & 511}\0${sha256File(path)}`);
  }
  for (const child of readdirSync2(root).sort()) visit(join4(root, child));
  return sha2562(entries.join("\n"));
}
function runtimeManifestPath(runtimeDirectory) {
  return join4(runtimeDirectory, "runtime-manifest.json");
}
function loadWorkerRuntimeManifest(runtimeDirectory, expected = {}) {
  const directory = resolve4(runtimeDirectory);
  const path = runtimeManifestPath(directory);
  if (!existsSync4(path)) return { valid: false, reason: "runtime-manifest-missing", directory };
  try {
    const manifest = JSON.parse(readFileSync3(path, "utf8"));
    const workerPath = join4(directory, "workflow-worker.mjs");
    const lockPath = join4(directory, "package-lock.json");
    const packagePath = join4(directory, "package.json");
    const platformPackagePath = join4(directory, "node_modules", ...String(manifest.platform_package ?? "").split("/"), "package.json");
    const sdkPackagePath = join4(directory, "node_modules", "@cursor", "sdk", "package.json");
    const reasons = [];
    ...
    for (const candidate of [workerPath, lockPath, packagePath, platformPackagePath, sdkPackagePath]) if (!existsSync4(candidate)) reasons.push(`runtime-file-missing:${basename2(candidate)}`);
    ...
    if (reasons.length === 0) {
      const sdkPackage = JSON.parse(readFileSync3(sdkPackagePath, "utf8"));
      const platformPackage = JSON.parse(readFileSync3(platformPackagePath, "utf8"));
    ...
    const runtimeHash = sha2562(JSON.stringify({
...
function bundledWorkerEntrypoint() {
  const candidates = [
    fileURLToPath2(new URL("./workflow-worker.mjs", import.meta.url)),
    fileURLToPath2(new URL("../../dist/workflow-worker.mjs", import.meta.url))
  ];
...
function resolveWorkerRuntime({ workerEntrypoint, runtimeRoot, pluginRoot: pluginRoot2 } = {}) {
  ...
  if (process.env.GELDMACHER_WORKFLOW_WORKER) return {
    entrypoint: resolve5(process.env.GELDMACHER_WORKFLOW_WORKER),
  ...
  const provisioned = loadWorkerRuntimeManifest(runtimeDirectory, {
    ...
    ...pluginRoot2 ? { plugin_hash: hashPluginTree(pluginRoot2) } : {}
  });
...
var CursorWorkerAdapter = class {
  constructor({ runDirectory, workerEntrypoint, runtimeRoot, pluginRoot: pluginRoot2, sandbox = runSandboxedProcess } = {}) {
    this.runDirectory = resolve5(runDirectory);
    this.workerRuntime = resolveWorkerRuntime({ workerEntrypoint, runtimeRoot, pluginRoot: pluginRoot2 });
    this.workerEntrypoint = this.workerRuntime.entrypoint;
    this.sandbox = sandbox;
    mkdirSync4(this.runDirectory, { recursive: true, mode: 448 });
  }
  controlPath() {
    return join5(this.runDirectory, "worker-control.json");
  }
  runtimeProvenance() {
    return {
  ...
  workerHome() {
    const path = join5(this.runDirectory, "worker-home");
    mkdirSync4(path, { recursive: true, mode: 448 });
    return path;
  }
  controllerStatePaths() {
    return ["run.json", "preparation.json", "events.jsonl", ".lock"].map((name) => join5(this.runDirectory, name));
  }
  listModels() {
  ...
  runPhase({ role, route, routePoolHash = null, selectionReason = "primary-available", acceptedModel, prompt, cwd, mode = "agent", agentId = null, force = false, writerWritablePaths = [], writerDeniedPaths = [], verifierArtifactPaths = [], timeoutMs = 3e5, cancelGraceMs = 5e3, configurationHash = null, harnessHash = null, artifactProjectionHash = null }) {
    ...
    const receipt = {
      ...
      sdk_version: sdkVersion,
      configuration_hash: configurationHash ?? createHash4("sha256").update(JSON.stringify(route)).digest("hex"),
      route_hash: configurationHash ?? createHash4("sha256").update(JSON.stringify(route)).digest("hex"),
...
function loadCapabilityReceipt(stateRoot2, expected = {}) {
  const path = capabilityReceiptPath(stateRoot2);
...
function resolveCapabilities(stateRoot2, additions = {}, context = {}) {
  ...
  const receipt = runtime.valid && cursorVersion ? loadCapabilityReceipt(stateRoot2, expected) : null;
  const output = {
...
function defaultProjectPolicyPath(workspaceRoot) {
  return join7(resolve6(workspaceRoot), ".cursor", "workflow-policy.yaml");
}
function readYaml(path) {
  if (!existsSync7(path)) return null;
  const value = (0, import_yaml2.parse)(readFileSync5(path, "utf8"));
...
function createRunWorktree(workspaceRoot, runId2, options = {}) {
  const root = resolve7(options.root ?? defaultWorktreeRoot(workspaceRoot));
  const path = join8(root, runId2);
...
var dependencyManifestNames = /* @__PURE__ */ new Set(["package.json", "package-lock.json", "pnpm-lock.yaml", "yarn.lock", "bun.lock", "bun.lockb", "requirements.txt", "pyproject.toml", "poetry.lock", "Pipfile", "Pipfile.lock", "Gemfile", "Gemfile.lock", "Cargo.toml", "Cargo.lock", "go.mod", "go.sum"]);
...
function detectDependencyChanges(worktreePath, baseline, paths) {
  ...
  for (const path of packagePaths) {
    ...
    let after = {};
    try {
      after = JSON.parse(readFileSync6(assertContainedPath(worktreePath, path), "utf8"));
    } catch {
      after = {};
    }
...
function assertContainedPath(root, candidate) {
  ...
  const normalizedCandidate = resolve7(root, candidate);
  const rel = relative2(normalizedRoot, normalizedCandidate);
  if (rel === "" || !rel.startsWith(`..${sep3}`) && rel !== ".." && !isAbsolute2(rel)) return normalizedCandidate;
...
function runHostCheck(worktreePath, command, timeoutMs = 3e5) {
  ...
  const temporary = mkdtempSync2(join8(tmpdir2(), "workflow-host-check-"));
  const started = Date.now();
  try {
    const profile = join8(temporary, "host-check.sb");
    writeFileSync5(profile, buildSandboxProfile({ writablePaths: [temporary], network: false }), { mode: 384 });
    const executable = process.platform === "darwin" && existsSync8("/usr/bin/sandbox-exec") ? "/usr/bin/sandbox-exec" : null;
...
function rootProjection(rootPlanText, pluginRoot2) {
  const inspection = inspectArtifactText(rootPlanText, pluginRoot2);
...
function semanticRootDiff(beforeText, afterText, pluginRoot2) {
  ...
  const before = rootProjection(beforeText, pluginRoot2);
...
function loadPlanningHarness(pluginRoot2) {
  const sources = harnessFiles.map((path) => {
    const absolute = join9(pluginRoot2, path);
...
var PlanningEngine = class {
  constructor({ workspaceRoot, store: store2, pluginRoot: pluginRoot2, stateRoot: stateRoot2, adapterFactory, capabilitiesFactory } = {}) {
    this.workspaceRoot = resolve8(workspaceRoot);
    this.store = store2;
    this.pluginRoot = resolve8(pluginRoot2);
    this.stateRoot = resolve8(stateRoot2);
  ...
  execute(preparationId2) {
    let preparation = this.store.get(preparationId2);
    assertCompatiblePreparation(preparation);
...
function planningHarnessHash(pluginRoot2) {
  return loadPlanningHarness(resolve8(pluginRoot2)).hash;
}
...
var WorkflowEngine = class {
  ...
  start({ preparationId: preparationId2, approvedRootHash, expectedPreparationRevision, idempotencyKey }) {
    ...
    const preparation = this.preparationStore.get(preparationId2);
    assertCompatiblePreparation(preparation);
    const prior = this.store.list().find((run) => run.preparation_id === preparationId2 && run.start_idempotency_key === idempotencyKey);
  ...
  update(runId2, mutator, eventType) {
