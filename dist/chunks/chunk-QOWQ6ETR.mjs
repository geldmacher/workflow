...
`;
  ...
  appendFileSync(eventPath, line, { mode: 384 });
  atomicJson(headPath, {
    schema: 1,
    count: head.count + 1,
...
var RunStore = class {
  ...
  createFromPreparation(preparationStore, options, input) {
    ...
    try {
      ...
      if (existing) {
        ...
        const preparation2 = preparationStore.get(options.preparationId);
        if (preparation2.status !== "consumed") preparationStore.consume(preparation2.preparation_id, preparation2.revision, existing.run_id, options.idempotencyKey);
  ...
  qualifyingHistory(qualificationKey = null) {
    return this.index().subjects.filter((run) => classifyRunCompatibility(run).compatible && run.lifecycle === "achieved" && run.effective_profile === "supervised" && run.root_review_complete === true && run.review_assessment === "achieved" && run.blocker_count === 0 && run.delivery_accepted === true && run.evidence_grade === "verified" && (!qualificationKey || run.qualification_key === qualificationKey)).length;
  }
};
