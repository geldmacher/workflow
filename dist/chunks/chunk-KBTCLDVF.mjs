...
var ArtifactHandoffStore = class {
  ...
  loadIndex({ repair = false } = {}) {
    try {
      ...
      const actual = existsSync(this.directory) ? readdirSync(this.directory, { withFileTypes: true }).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => entry.name.slice(0, -5)).sort() : [];
  ...
  quarantineArtifact(artifactId, { expectedTextHash, apply = false, now = () => /* @__PURE__ */ new Date() } = {}) {
    ...
    try {
      ...
      if (existsSync(indexPath)) writeFileSync(join(quarantineDirectory, "index-before.json"), readFileSync(indexPath), { mode: 384, flag: "wx" });
