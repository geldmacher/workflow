...
function changedPathsBetween(workspaceRoot, fromRef, toRef = "HEAD") {
  ...
  return git(workspaceRoot, ["diff", "--no-renames", "--name-only", "-z", `${fromRef}..${toRef}`], { raw: true }).split("\0").filter(Boolean).sort();
}
function gitStatus(workspace, args) {
  const result = spawnSync("git", ["-C", workspace, ...args], { encoding: "utf8", timeout: 12e4 });
...
function workspaceDeliveryMatch(workspaceRoot, deliveryCommit, deliveredPaths = []) {
  ...
  try {
    recreatedDeletion = paths.some((path) => {
      ...
      try {
        lstatSync(assertContainedPath(workspaceRoot, path));
        return true;
      } catch (error) {
  ...
  return ancestor.status === 0 ? { status: "drifted", matched: false, paths, reason: "integrated-content-drifted" } : { status: "not-integrated", matched: false, paths, reason: "delivered-content-not-present" };
}
