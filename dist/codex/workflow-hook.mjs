...
var require_stringifyPair = __commonJS({
  "node_modules/yaml/dist/stringify/stringifyPair.js"(exports) {
    "use strict";
    var identity = require_identity(), Scalar = require_Scalar(), stringify2 = require_stringify(), stringifyComment = require_stringifyComment();
...
var require_addPairToJSMap = __commonJS({
  "node_modules/yaml/dist/nodes/addPairToJSMap.js"(exports) {
    "use strict";
    var log = require_log(), merge = require_merge(), stringify2 = require_stringify(), identity = require_identity(), toJS = require_toJS();
...
var require_stringifyCollection = __commonJS({
  "node_modules/yaml/dist/stringify/stringifyCollection.js"(exports) {
    "use strict";
    var identity = require_identity(), stringify2 = require_stringify(), stringifyComment = require_stringifyComment();
...
var require_stringifyDocument = __commonJS({
  "node_modules/yaml/dist/stringify/stringifyDocument.js"(exports) {
    ...
    function stringifyDocument(doc, options) {
      ...
      let ctx = stringify2.createStringifyContext(doc, options), { commentString } = ctx.options;
      ...
      if (doc.contents) {
        ...
        let onChompKeep = contentComment ? void 0 : () => chompKeep = !0, body = stringify2.stringify(doc.contents, ctx, () => contentComment = null, onChompKeep);
...
function stringList(value, label, { required = !1 } = {}) {
...
function validateVerification(value, acceptanceCount) {
  ...
  return value.map((entry, index) => {
    ...
    let allowed = ["check_id", "objectives", "verification_intent", "expected_evidence", "required", "evidence_class", "cost_class", "prerequisites"], unknown = Object.keys(entry).filter((field) => !allowed.includes(field));
    if (unknown.length > 0) throw new Error(`workflow authority verification[${index}] contains unsupported field ${unknown[0]}`);
    let checkId = requiredLine(entry.check_id, `workflow authority verification[${index}].check_id`);
    ...
    return {
      ...
      prerequisites: stringList(entry.prerequisites, `workflow authority verification ${checkId}.prerequisites`, { required: !0 })
    };
  });
}
function normalizeAuthorityCore(input, { planMarkdown = null, requireHash = !0 } = {}) {
  ...
  let allowed = [
    ...
    "certification",
    "extensions"
  ], unknown = Object.keys(input).filter((field) => !allowed.includes(field));
...
function preflightRootPlan(text, root = defaultRoot) {
  ...
  if (inspected.errors.length > 0 || parsed?.fields?.artifact !== "work-plan" || parsed.fields.schema !== 6)
    return {
      ...
      blocking_issues: (inspected.errors.length > 0 ? inspected.errors : ["input must be one Schema-6 work-plan"]).map((message) => issue("invalid-root", message)),
      advisories: [],
      required_checks: [],
      deferred_checks: [],
...
function rootPlanFingerprint(rootPlanText) {
  return createHash4("sha256").update(normalizeRootPlanText(rootPlanText)).digest("hex");
}
function unwrapProposedPlanInterior(interior) {
  let body = String(interior ?? "");
  return body.startsWith(`\r
`) ? body = body.slice(2) : body.startsWith(`
`) && (body = body.slice(1)), body.endsWith(`\r
`) ? body = body.slice(0, -2) : body.endsWith(`
`) && (body = body.slice(0, -1)), body;
}
function extractRootPlanText(source) {
  let text = String(source ?? ""), proposedMatch = text.match(/<proposed_plan>([\s\S]*?)<\/proposed_plan>/i), proposed = proposedMatch ? unwrapProposedPlanInterior(proposedMatch[1]) : text.trim();
  try {
...
function parseRootPlanFields(rootPlanText) {
  if (typeof rootPlanText != "string" || !rootPlanText.trim())
    return { ok: !1, reason: "missing-root-text", fields: null, fingerprint: null, content_hash: null };
  let match = rootPlanText.match(/^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n([\s\S]*))?$/);
  if (!match) return { ok: !1, reason: "missing-frontmatter", fields: null, fingerprint: null, content_hash: null };
  let fields;
  try {
    fields = (0, import_yaml3.parse)(match[1]);
  } catch {
    return { ok: !1, reason: "invalid-frontmatter", fields: null, fingerprint: null, content_hash: null };
  }
  return asObject(fields) ? fields.artifact !== "work-plan" || fields.schema !== 6 ? { ok: !1, reason: "not-schema6-work-plan", fields, fingerprint: null, content_hash: null } : typeof fields.id != "string" || !ROOT_ID.test(fields.id) ? { ok: !1, reason: "invalid-root-id", fields, fingerprint: null, content_hash: null } : ["low", "medium", "high"].includes(fields.risk) ? Array.isArray(fields.hard_triggers) ? {
    ok: !0,
    reason: null,
  ...
  } : { ok: !1, reason: "invalid-hard-triggers", fields, fingerprint: null, content_hash: null } : { ok: !1, reason: "invalid-risk", fields, fingerprint: null, content_hash: null } : { ok: !1, reason: "invalid-frontmatter", fields: null, fingerprint: null, content_hash: null };
}
function inspectPresentedRootPlan(rootPlanText, options = {}) {
  let parsed = parseRootPlanFields(rootPlanText);
  if (!parsed.ok)
    return {
      ok: !1,
  ...
  let preflight = (options.preflightRootPlan ?? preflightRootPlan)(
    rootPlanText,
    options.pluginRoot ?? defaultRoot
  );
  if (!preflight?.feasible) {
    let blockers = Array.isArray(preflight?.blocking_issues) ? preflight.blocking_issues : ["native-preflight-unavailable"];
    return {
      ok: !1,
...
function explicitWorkflowCommand(prompt) {
  ...
  for (let match of text.matchAll(WORKFLOW_TOKEN)) commands.push(match[1].toLowerCase());
  let unique2 = [...new Set(commands)];
  return { command: unique2.length === 1 ? unique2[0] : null, ambiguous: unique2.length > 1 };
}
function explicitCodexCollaborationMode(input) {
  let value = input?.collaboration_mode;
  return value && typeof value == "object" && !Array.isArray(value) && typeof value.mode == "string" ? value.mode.trim().toLowerCase() : null;
}
function classifyCodexWorkflowPrompt(prompt) {
  if (hookContinuation(prompt)) return { kind: "hook-continuation", phase: null };
  let explicit = explicitWorkflowCommand(prompt);
  return explicit.ambiguous ? { kind: "ambiguous-workflow-skill", phase: null } : explicit.command === "plan-work" ? { kind: "workflow-skill", phase: "planning" } : explicit.command === "correct-work" ? { kind: "workflow-skill", phase: "correction" } : explicit.command === "review-work" ? { kind: "workflow-skill", phase: "review" } : explicit.command ? { kind: "workflow-skill", phase: explicit.command.replace(/-work$/, "") } : { kind: "ordinary", phase: null };
}
function planningStop(input, state, options = {}) {
  let message = String(input.last_assistant_message ?? ""), rootText = extractRootPlanText(message), reason = null;
  if (!/<proposed_plan>[\s\S]*<\/proposed_plan>/i.test(message) || !rootText)
    ...
    let inspected = inspectPresentedRootPlan(rootText, { pluginRoot: options.pluginRoot, preflightRootPlan: options.preflightRootPlan });
    if (!inspected.ok) {
      let detail = inspected.blockers.slice(0, 4).map((issue2) => String(issue2?.message ?? issue2).replace(/\s+/g, " ").slice(0, 200)).filter(Boolean).join("; ");
      reason = `Workflow Plan validation failed: the native Plan must contain one valid Schema-6 Root${detail ? `: ${detail}` : "."}`;
    }
  }
  return state.turn = null, reason ? input.stop_hook_active === !0 ? { output: { continue: !1, stopReason: "Workflow native Plan validation failed.", systemMessage: reason }, state } : { output: { decision: "block", reason }, state } : { output: {}, state };
}
function freshState(priorState = {}) {
  return priorState?.schema === 6 && priorState?.kind === "workflow-lifecycle-kernel" ? structuredClone(priorState) : { schema: 6, kind: "workflow-lifecycle-kernel", turn: null };
}
function evaluateCodexHook(input, priorState = {}, options = {}) {
  let state = freshState(priorState), event = input?.hook_event_name;
  if (event === "UserPromptSubmit") {
    state.turn = null;
    let classification = classifyCodexWorkflowPrompt(input.prompt);
    if (classification.kind === "hook-continuation" || classification.kind === "ordinary") return { output: {}, state };
    if (classification.kind === "ambiguous-workflow-skill") return { output: { decision: "block", reason: "Workflow \xB7 Blocked. Use exactly one explicit Workflow skill in this prompt." }, state };
    if (classification.phase === "planning") {
      let mode = explicitCodexCollaborationMode(input);
    ...
    let marker = classification.phase === "planning" ? CODEX_PLAN_MARKER : classification.phase === "review" ? CODEX_REVIEW_MARKER : ["implementation", "correction"].includes(classification.phase) ? CODEX_IMPLEMENTATION_MARKER : "[workflow-codex-manual-v6]", phaseBoundary = classification.phase === "review" ? "Review is conceptually repository-read-only. The active project harness chooses and enforces concrete execution and attests before/after snapshots; Workflow does not classify tools or commands." : "The active project harness owns every concrete execution choice; Workflow owns only lifecycle, authority, lineage, evidence grade, and human gates.";
    return {
      output: { hookSpecificOutput: { hookEventName: "UserPromptSubmit", additionalContext: `${marker} ${phaseBoundary}` } },
      state
    };
  }
  return event === "Stop" && state.turn?.phase === "planning" ? planningStop(input, state, options) : (event === "Stop" && (state.turn = null), { output: {}, state });
}

// src/hosts/codex/workflow-hook.mjs
var MAX_INPUT_BYTES = 1024 * 1024, digest = (value) => createHash5("sha256").update(String(value)).digest("hex");
...
function unavailableOutput(input, error) {
  let reason = `Workflow hook unavailable: ${String(error?.message ?? error).slice(0, 400)}. Host-native tools remain available; do not claim verified Workflow evidence from this turn.`;
...
function deactivateBestEffort(input, options) {
  try {
    let path = statePath(input, options.stateRoot);
    if (!existsSync3(path)) return;
    let state = readState(path);
    state.turn = null, writeState(path, state);
  } catch {
  }
}
...
function sameTurn(state, input) {
  return !!(state?.turn?.turn_id && input.turn_id && state.turn.turn_id === input.turn_id);
}
function runCodexHook(input, options = {}) {
  ...
  if (event === "UserPromptSubmit") {
    ...
    if (inactivePrompt(classification))
      return deactivateBestEffort(input, options), {};
  }
  let path, state;
  try {
    path = statePath(input, options.stateRoot), state = readStateOrEmpty(path);
  } catch (error) {
    return event === "UserPromptSubmit" ? unavailableOutput(input, error) : {};
  }
  if (event !== "UserPromptSubmit" && !sameTurn(state, input)) return {};
  try {
    let pluginRoot = options.pluginRoot ?? resolveCodexPluginRoot(), evaluated = evaluateCodexHook(input, state, { ...options, pluginRoot });
    return writeState(path, evaluated.state), evaluated.output;
  } catch (error) {
