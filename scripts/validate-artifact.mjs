...
function validateArtifactSet(entries, root = defaultRoot) {
...
function repositoryPathFingerprint(workspaceRoot, repositoryPath) {
  ...
  try {
    ...
    if (stat.isFile()) return `file:${stat.mode.toString(8)}:${sha2562(readFileSync3(absolute))}`;
...
function assertSafeDirectory(path, base) {
  ...
  while (current !== resolvedBase && !existsSync3(current)) current = dirname2(current);
...
function invalidateManualCheckReceipts({ rootPlanText, workspaceRoot, options = {} }) {
  ...
  if (!existsSync3(base)) return false;
...
function readTipFile(path, rootPlanId) {
  ...
  return validateTip(JSON.parse(readFileSync5(path, "utf8")), rootPlanId);
}
...
var ArtifactHandoffStore = class {
  ...
  loadIndex({ repair = false } = {}) {
    try {
      const index = JSON.parse(readFileSync5(this.indexPath(), "utf8"));
      const actual = existsSync4(this.directory) ? readdirSync2(this.directory, { withFileTypes: true }).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => entry.name.slice(0, -5)).sort() : [];
  ...
  records(ids2 = null) {
    if (!existsSync4(this.directory)) return [];
    const files = ids2 ? [...new Set(ids2)].map((id) => this.artifactPath(id)).filter(existsSync4) : readdirSync2(this.directory, { withFileTypes: true }).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => join6(this.directory, entry.name));
    return files.map((path) => validateRecord(JSON.parse(readFileSync5(path, "utf8")))).sort((left, right) => left.artifact_id.localeCompare(right.artifact_id));
  }
  record(artifacts) {
  ...
  quarantineArtifact(artifactId, { expectedTextHash, apply = false, now = () => /* @__PURE__ */ new Date() } = {}) {
    ...
    try {
      ...
      if (existsSync4(indexPath)) writeFileSync2(join6(quarantineDirectory, "index-before.json"), readFileSync5(indexPath), { mode: 384, flag: "wx" });
...
function assertRepositoryContainment(repositoryRoot, repositoryPath) {
  ...
  while (!existsSync5(existing) && existing !== root) existing = dirname4(existing);
