  ...
  "node_modules/yaml/dist/compose/resolve-block-map.js"(exports) {
    ...
    function resolveBlockMap({ composeNode, composeEmptyNode }, ctx, bm, onError, tag) {
      ...
      for (let collItem of bm.items) {
        let { start, key, sep: sep3, value } = collItem, keyProps = resolveProps.resolveProps(start, {
        ...
        if (implicitKey) {
          if (key && (key.type === "block-seq" ? onError(offset, "BLOCK_AS_IMPLICIT_KEY", "A block sequence may not be used as an implicit map key") : "indent" in key && key.indent !== bm.indent && onError(offset, "BAD_INDENT", startColMsg)), !keyProps.anchor && !keyProps.tag && !sep3) {
...
` + keyProps.comment : map.comment = keyProps.comment);
        ...
        let valueProps = resolveProps.resolveProps(sep3 ?? [], {
        ...
        if (offset = valueProps.end, valueProps.found) {
          ...
          let valueNode = value ? composeNode(ctx, value, valueProps, onError) : composeEmptyNode(ctx, offset, sep3, null, valueProps, onError);
        ...
        for (let token of end) {
          ...
          switch (type) {
            ...
            case "comment": {
              ...
              comment ? comment += sep3 + cb : comment = cb, sep3 = "";
              break;
            }
            case "newline":
              comment && (sep3 += source), hasSpace = !0;
              break;
            default:
  ...
  "node_modules/yaml/dist/compose/resolve-flow-collection.js"(exports) {
    ...
    function resolveFlowCollection({ composeNode, composeEmptyNode }, ctx, fc, onError, tag) {
      ...
      for (let i = 0; i < fc.items.length; ++i) {
        let collItem = fc.items[i], { start, key, sep: sep3, value } = collItem, props = resolveProps.resolveProps(start, {
          flow: fcName,
          indicator: "explicit-key-ind",
          next: key ?? sep3?.[0],
          offset,
...
` + prevItemComment : prev.comment = prevItemComment, props.comment = props.comment.substring(prevItemComment.length + 1);
        ...
        if (!isMap && !sep3 && !props.found) {
          let valueNode = value ? composeNode(ctx, value, props, onError) : composeEmptyNode(ctx, props.end, sep3, null, props, onError);
          ...
          let valueNode = value ? composeNode(ctx, value, valueProps, onError) : valueProps.found ? composeEmptyNode(ctx, valueProps.end, sep3, null, valueProps, onError) : null;
...
`.repeat(Math.max(1, lines.length - 1)) : "", end2 = start + header.length;
      ...
      let value = "", sep3 = "", prevMoreIndented = !1;
...
`;
      for (let i = contentStart; i < chompStart; ++i) {
        ...
        type === Scalar.Scalar.BLOCK_LITERAL ? (value += sep3 + indent.slice(trimIndent) + content, sep3 = `
`) : indent.length > trimIndent || content[0] === "	" ? (sep3 === " " ? sep3 = `
` : !prevMoreIndented && sep3 === `
` && (sep3 = `

`), value += sep3 + indent.slice(trimIndent) + content, sep3 = `
`, prevMoreIndented = !0) : content === "" ? sep3 === `
` ? value += `
` : sep3 = `
` : (value += sep3 + content, sep3 = " ", prevMoreIndented = !1);
      }
      switch (header.chomp) {
        case "-":
          break;
...
`, "sy");
      ...
      if (!match)
        return source;
      let res = match[1], sep3 = " ", pos = first.lastIndex;
      for (line.lastIndex = pos; match = line.exec(source); )
        match[1] === "" ? sep3 === `
` ? res += sep3 : sep3 = `
` : (res += sep3 + match[1], sep3 = " "), pos = line.lastIndex;
      let last = /[ \t]*(.*)/sy;
      return last.lastIndex = pos, match = last.exec(source), res + sep3 + (match?.[1] ?? "");
    }
...
var require_cst_stringify = __commonJS({
  "node_modules/yaml/dist/parse/cst-stringify.js"(exports) {
    ...
    function stringifyItem({ start, key, sep: sep3, value }) {
      ...
      for (let st of start)
        res += st.source;
      if (key && (res += stringifyToken(key)), sep3)
        for (let st of sep3)
          res += st.source;
    ...
    var Parser = class {
      ...
      *scalar(scalar) {
        if (this.type === "map-value-ind") {
          let prev = getPrevProps(this.peek(2)), start = getFirstKeyStartProps(prev), sep3;
          scalar.end ? (sep3 = scalar.end, sep3.push(this.sourceToken), delete scalar.end) : sep3 = [this.sourceToken];
          let map = {
            type: "block-map",
              ...
              if (it.explicitKey)
                if (it.sep)
                  if (it.value)
                    ...
                    let start2 = getFirstKeyStartProps(it.start), key = it.key, sep3 = it.sep;
                    sep3.push(this.sourceToken), delete it.key, delete it.sep, this.stack.push({
                      type: "block-map",
                      offset: this.offset,
                      indent: this.indent,
      ...
      *flowCollection(fc) {
        ...
        if (this.type === "flow-error-end") {
          ...
          if (parent.type === "block-map" && (this.type === "map-value-ind" && parent.indent === fc.indent || this.type === "newline" && !parent.items[parent.items.length - 1].sep))
            ...
            fixFlowSeqItems(fc);
            let sep3 = fc.end.splice(1, fc.end.length);
...
var require_fast_uri = __commonJS({
  "node_modules/fast-uri/index.js"(exports, module) {
    ...
    function resolveComponent(base, relative4, options, skipNormalization) {
      let target = {};
      return skipNormalization || (base = parse2(serialize(base, options), options), relative4 = parse2(serialize(relative4, options), options)), options = options || {}, !options.tolerant && relative4.scheme ? (target.scheme = relative4.scheme, target.userinfo = relative4.userinfo, target.host = relative4.host, target.port = relative4.port, target.path = removeDotSegments(relative4.path || ""), target.query = relative4.query) : (relative4.userinfo !== void 0 || relative4.host !== void 0 || relative4.port !== void 0 ? (target.userinfo = relative4.userinfo, target.host = relative4.host, target.port = relative4.port, target.path = removeDotSegments(relative4.path || ""), target.query = relative4.query) : (relative4.path ? (relative4.path[0] === "/" ? target.path = removeDotSegments(relative4.path) : ((base.userinfo !== void 0 || base.host !== void 0 || base.port !== void 0) && !base.path ? target.path = "/" + relative4.path : base.path ? target.path = base.path.slice(0, base.path.lastIndexOf("/") + 1) + relative4.path : target.path = relative4.path, target.path = removeDotSegments(target.path)), target.query = relative4.query) : (target.path = base.path, relative4.query !== void 0 ? target.query = relative4.query : target.query = base.query), target.userinfo = base.userinfo, target.host = base.host, target.port = base.port), target.scheme = base.scheme), target.fragment = relative4.fragment, target;
    }
...
import { basename, dirname, resolve as resolve4 } from "node:path";
...
function parsePlanContainer(text, wrapper, failures, normalizations = []) {
  ...
  if (!match)
    return failures.push("native plan must contain one H1 followed by a yaml artifact-envelope"), null;
  let fields = yamlObject(match[2], "artifact envelope", failures);
...
`) ? 1 : 0) > 0 && normalizations.push("ignored Cursor progress text before native plan"), { fields, body: String(text).slice(match.index + match[0].length), wrapper, container: "cursor-plan", title: match[1], normalizations }) : null;
}
...
}), CLOSEOUT_INPUT_KIND = "closeout-input", CLOSEOUT_INPUT_PHASES = Object.freeze([
  "implementation",
  "correction",
  "review-recovery"
]), DELIVERY_REPORT_KIND = "delivery-report";
var EVIDENCE_ID = /^de-[A-Za-z0-9][A-Za-z0-9-]*$/, ROOT_ID = /^wp-[A-Za-z0-9][A-Za-z0-9-]*$/;
var FINAL_STEP_HEADING = /^##\s+Final implementation step\s*$/im, ATTESTATION_FENCE_OPEN = /^```yaml workflow-attestation\s*$/, ATTESTATION_FENCE_CLOSE = /^```\s*$/, BACKTICK_FENCE_OPEN = /^```(?:ya?ml|markdown|md)?\s*$/i;
...
function isPlanCloseoutAttestation(value) {
  ...
  return object ? object.schema === PLAN_CLOSEOUT_ATTESTATION.schema && object.kind === PLAN_CLOSEOUT_ATTESTATION.kind && [PLAN_CLOSEOUT_ATTESTATION.action, LEGACY_PLAN_CLOSEOUT_ATTESTATION.action].includes(object.action) && Object.keys(object).length === 3 : !1;
}
var CLOSEOUT_INPUT_FIELDS = Object.freeze([
  ...
  "repetitions",
  "limitations"
]), CHECK_ID = /^CHECK-[1-9][0-9]*$/, EVIDENCE_GRADES = /* @__PURE__ */ new Set(["verified", "supported", "partial", "unavailable", "failed"]);
function exactKeys(value, allowed) {
  return Object.keys(value).filter((key) => !allowed.includes(key));
}
function isSafeRepositoryPath(value) {
  return typeof value != "string" || value.length === 0 || value !== value.trim() || value.startsWith("/") || value.includes("\\") || value.includes("\0") ? !1 : value.split("/").every((segment) => segment !== "" && segment !== "." && segment !== "..");
}
function validateCloseoutCheckEvidence(entries, issues) {
  ...
  entries.length > 128 && issues.push("closeout-input check_evidence must contain at most 128 entries");
  let ids2 = /* @__PURE__ */ new Set();
  for (let [index, entry] of entries.entries()) {
    let object = asObject(entry);
    if (!object) {
      issues.push(`closeout-input check_evidence[${index}] must be an object`);
      continue;
    }
    let unknown = exactKeys(object, CHECK_EVIDENCE_FIELDS);
    unknown.length > 0 && issues.push(`closeout-input check_evidence[${index}] has unknown fields: ${unknown.join(", ")}`), CHECK_ID.test(String(object.check_id ?? "")) ? ids2.has(object.check_id) ? issues.push(`closeout-input check_evidence contains duplicate ${object.check_id}`) : ids2.add(object.check_id) : issues.push(`closeout-input check_evidence[${index}] requires a valid CHECK-* check_id`), EVIDENCE_GRADES.has(object.grade) || issues.push(`closeout-input ${object.check_id ?? `check_evidence[${index}]`} has an invalid grade`), (typeof object.observed != "string" || object.observed.trim().length === 0 || object.observed.length > 1e4) && issues.push(`closeout-input ${object.check_id ?? `check_evidence[${index}]`} requires a non-empty observed result`), (!Number.isInteger(object.repetitions) || object.repetitions < 0) && issues.push(`closeout-input ${object.check_id ?? `check_evidence[${index}]`} requires non-negative integer repetitions`);
    for (let field of ["feature_id", "surface", "method", "expected"])
      Object.prototype.hasOwnProperty.call(object, field) && object[field] !== null && typeof object[field] != "string" && issues.push(`closeout-input ${object.check_id ?? `check_evidence[${index}]`} ${field} must be a string or null`);
    ["feature_id", "surface", "method", "expected"].some((field) => typeof object[field] == "string" && object[field].length > 1e4) && issues.push(`closeout-input ${object.check_id ?? `check_evidence[${index}]`} string fields must not exceed 10000 characters`);
    for (let field of ["limitations"])
      Object.prototype.hasOwnProperty.call(object, field) && !Array.isArray(object[field]) && issues.push(`closeout-input ${object.check_id ?? `check_evidence[${index}]`} ${field} must be an array`);
    Array.isArray(object.limitations) && object.limitations.some((item) => typeof item != "string" || !item.trim()) && issues.push(`closeout-input ${object.check_id ?? `check_evidence[${index}]`} limitations must contain non-empty strings`), Array.isArray(object.limitations) && (object.limitations.length > 64 || object.limitations.some((item) => item.length > 2e3)) && issues.push(`closeout-input ${object.check_id ?? `check_evidence[${index}]`} limitations exceed the bounded report size`);
  }
}
function validateCloseoutInput(value) {
  let report = asObject(value), issues = [];
  if (!report) return { ok: !1, issues: ["closeout-input must be an object"], report: null };
  let unknown = exactKeys(report, CLOSEOUT_INPUT_FIELDS);
  return unknown.length > 0 && issues.push(`closeout-input has unknown fields: ${unknown.join(", ")}`), report.schema !== 1 && issues.push("closeout-input schema must be 1"), report.kind !== CLOSEOUT_INPUT_KIND && issues.push("closeout-input kind must be closeout-input"), CLOSEOUT_INPUT_PHASES.includes(report.phase) || issues.push("closeout-input phase must be implementation, correction, or review-recovery"), ROOT_ID.test(String(report.root_plan_id ?? "")) || issues.push("closeout-input root_plan_id must be a valid wp-* ID"), (!Number.isInteger(report.strategy_revision) || report.strategy_revision < 0) && issues.push("closeout-input strategy_revision must be a non-negative integer"), Object.prototype.hasOwnProperty.call(report, "changed_paths") && !Array.isArray(report.changed_paths) ? issues.push("closeout-input changed_paths must be an array when supplied as a non-authoritative hint") : Array.isArray(report.changed_paths) && (report.changed_paths.length > 1e3 && issues.push("closeout-input changed_paths must contain at most 1000 entries"), new Set(report.changed_paths).size !== report.changed_paths.length && issues.push("closeout-input changed_paths must be unique"), report.changed_paths.some((path) => !isSafeRepositoryPath(path) || path.length > 1e3) && issues.push("closeout-input changed_paths must contain normalized repository-relative paths"), [...report.changed_paths].sort().some((path, index) => path !== report.changed_paths[index]) && issues.push("closeout-input changed_paths must be sorted exactly")), validateCloseoutCheckEvidence(report.check_evidence, issues), (typeof report.summary != "string" || report.summary.trim().length === 0 || report.summary.length > 2e3) && issues.push("closeout-input summary must be a concise non-empty string of at most 2000 characters"), issues.length > 0 ? { ok: !1, issues, report: null } : { ok: !0, issues: [], report: structuredClone(report) };
}
function parseCloseoutInput(text, { expectedPhase = null } = {}) {
  let source = normalizeNewlines(text), attestations = extractWorkflowAttestations(source), candidates = attestations.filter((entry) => entry.value?.kind === CLOSEOUT_INPUT_KIND), issues = [];
  candidates.length !== 1 && issues.push("response must contain exactly one unindented yaml workflow-attestation closeout-input block"), attestations.filter((entry) => entry.value?.schema === 1).length !== 1 && issues.push("response must not contain another operative Schema-1 workflow-attestation block");
  let remainder = source;
  for (let entry of [...attestations].reverse()) {
    let lines = remainder.split(`
`);
    remainder = [...lines.slice(0, entry.startLine), ...lines.slice(entry.endLine + 1)].join(`
`);
  }
  if ((/\b(?:do\s+not|don't|ignore|negate|skip)\b[\s\S]{0,120}\b(?:closeout-input|workflow-attestation)\b/i.test(remainder) || /\b(?:closeout-input|workflow-attestation)\b[\s\S]{0,120}\b(?:do\s+not|don't|ignore|negate|skip)\b/i.test(remainder) || /<!--[\s\S]*(?:closeout-input|workflow-attestation)[\s\S]*-->/i.test(source) || /~~[\s\S]*(?:closeout-input|workflow-attestation)[\s\S]*~~/i.test(source)) && issues.push("response must not negate, comment out, or strike through the closeout-input attestation"), candidates.length === 1) {
    let validated = validateCloseoutInput(candidates[0].value);
    if (issues.push(...validated.issues), validated.ok && expectedPhase && validated.report.phase !== expectedPhase && issues.push(`closeout-input phase must be ${expectedPhase}`), issues.length === 0) return { ok: !0, issues: [], report: validated.report };
  }
  return { ok: !1, issues: [...new Set(issues)], report: null };
}
function extractFinalImplementationStep(text) {
  let value = normalizeNewlines(text), match = value.match(FINAL_STEP_HEADING);
  if (!match || match.index == null) return null;
  let start = match.index + match[0].length, rest = value.slice(start), nextHeading = rest.search(/^##\s+/m);
...
`), attestations = [], inForeignFence = !1;
  for (let index = 0; index < lines.length; index += 1) {
    let line = lines[index];
    if (inForeignFence) {
      ATTESTATION_FENCE_CLOSE.test(line) && (inForeignFence = !1);
      continue;
    }
    ...
    let body = [], cursor = index + 1, closed = !1;
    for (; cursor < lines.length; ) {
      if (ATTESTATION_FENCE_CLOSE.test(lines[cursor])) {
        closed = !0;
        break;
      }
      body.push(lines[cursor]), cursor += 1;
    }
    if (closed) {
      try {
        let parsed = (0, import_yaml2.parse)(body.join(`
`));
        asObject(parsed) && attestations.push({
          value: parsed,
          startLine: index,
          endLine: cursor,
          raw: `${lines[index]}
...
function parsePlanCloseoutAttestationFromText(text, { requireFinalStepSection = !1, role = "instruction" } = {}) {
  let value = normalizeNewlines(text);
  if (requireFinalStepSection) {
    let sectionCount = countFinalImplementationSections(value);
    if (sectionCount === 0)
      return { ok: !1, issues: [`${role} must appear as an explicit ## Final implementation step section`] };
    if (sectionCount > 1)
      return { ok: !1, issues: [`${role} must include exactly one ## Final implementation step section`] };
    value = extractFinalImplementationStep(value) ?? "";
  }
  let attestations = extractWorkflowAttestations(value), planCloseouts = attestations.filter((entry) => isPlanCloseoutAttestation(entry.value)), issues = [];
  planCloseouts.length === 0 ? issues.push(`${role} must include exactly one unindented \`\`\`yaml workflow-attestation plan-closeout block`) : planCloseouts.length > 1 && issues.push(`${role} must include exactly one plan-closeout attestation`);
  let remainder = value;
  for (let entry of [...attestations].reverse()) {
    let lines = remainder.split(`
`);
...
`);
  ...
  return (/\[workflow-closeout-v\d+\]/i.test(remainder) || /\bworkflow_closeout\b/.test(remainder) || /\bexact Root\/chain\b/i.test(remainder)) && issues.push(`${role} must not include free-form closeout marker or workflow_closeout prose outside typed attestation`), (/\b(?:do\s+not|don't|does\s+not|shouldn'?t|ignore)\b[\s\S]{0,120}\b(?:attestation|closeout|workflow_closeout)\b/i.test(remainder) || /\b(?:attestation|closeout|workflow_closeout)\b[\s\S]{0,120}\b(?:do\s+not|don't|ignore)\b/i.test(remainder) || /<!--[\s\S]*workflow-attestation[\s\S]*-->/i.test(value) || /~~[\s\S]*workflow-attestation[\s\S]*~~/i.test(value)) && issues.push(`${role} must not negate or comment out the typed plan-closeout attestation`), issues.length > 0 ? { ok: !1, issues } : { ok: !0, issues: [], attestation: planCloseouts[0]?.value ?? null };
}
function todoPlanCloseoutIssues(todo, { role = "final native todo" } = {}) {
  ...
  let content = String(todo.content ?? "");
  return content.startsWith("[workflow-model-inherit-v1]") || issues.push(`${role} must start with [workflow-model-inherit-v1]`), /verify|check|evidence|snapshot|close\s*out/i.test(content) || issues.push(`${role} must verify or evidence the implemented result`), (/\[workflow-closeout-v\d+\]/i.test(content) || /\bworkflow_closeout\b/.test(content)) && issues.push(`${role} must keep closeout ceremony in workflow_attestation metadata, not todo prose`), isPlanCloseoutAttestation(todo.workflow_attestation) || issues.push(`${role} requires workflow_attestation: { schema: 1, kind: plan-closeout, action: delivery-closeout }; legacy workflow_closeout remains accepted`), issues;
}
function planCloseoutAttestationIssues(source, options = {}) {
  let { role = "instruction", requireFinalStepSection = !1 } = options;
  return asObject(source) && Object.prototype.hasOwnProperty.call(source, "workflow_attestation") ? todoPlanCloseoutIssues(source, { role }) : parsePlanCloseoutAttestationFromText(String(source ?? ""), { role, requireFinalStepSection }).issues;
}
function collectCloseoutStructuredCandidates(response) {
  let candidates = [];
  if (asObject(response?.structuredContent) && candidates.push(response.structuredContent), Array.isArray(response?.content))
    for (let entry of response.content)
      asObject(entry?.structuredContent) && candidates.push(entry.structuredContent);
  return candidates;
}
function expectedLineageFromArtifacts(artifacts = [], rootPlanId = null, options = {}) {
  let inspect = options.inspectArtifactText;
  if (typeof inspect != "function")
    throw new Error("expectedLineageFromArtifacts requires options.inspectArtifactText");
  let expectedSubject = rootPlanId, sourceReviewId = null, predecessorEvidenceId = null, entries = Array.isArray(artifacts) ? artifacts : [];
  for (let entry of entries) {
    let text = typeof entry == "string" ? entry : entry?.text;
    if (typeof text != "string") continue;
    let fields = inspect(text, options.pluginRoot).artifact?.fields;
    fields && (fields.artifact === "work-review" && fields.next_action === "correct" && typeof fields.correction_id == "string" && (expectedSubject = fields.correction_id, sourceReviewId = fields.id, typeof fields.latest_evidence_id == "string" && (predecessorEvidenceId = fields.latest_evidence_id)), fields.artifact === "delivery-evidence" && typeof fields.id == "string" && (predecessorEvidenceId = fields.id));
  }
...
function readCloseoutRecord(response, options = {}) {
  let candidates = collectCloseoutStructuredCandidates(response);
  if (candidates.length === 0) return { ok: !1, reason: "missing-structured-content", record: null };
  let first = JSON.stringify(candidates[0]);
  if (candidates.some((entry) => JSON.stringify(entry) !== first))
    return { ok: !1, reason: "conflicting-structured-content", record: null };
  let structured = candidates[0], id = typeof structured.delivery_evidence_id == "string" ? structured.delivery_evidence_id : "";
  if (!EVIDENCE_ID.test(id)) return { ok: !1, reason: "invalid-delivery-evidence-id", record: null };
  let rawArtifact = typeof structured.artifact == "string" && structured.artifact.length > 0 ? structured.artifact : null;
  if (!rawArtifact) return { ok: !1, reason: "missing-artifact", record: null };
  if (typeof structured.artifact_hash != "string" || !/^[a-f0-9]{64}$/.test(structured.artifact_hash))
    return { ok: !1, reason: "missing-artifact-hash", record: null };
  if (typeof structured.root_plan_id != "string" || !ROOT_ID.test(structured.root_plan_id))
    return { ok: !1, reason: "missing-root-plan-id", record: null };
  if (typeof structured.status != "string")
    return { ok: !1, reason: "missing-status", record: null };
  if (typeof structured.handoff_persisted != "boolean")
    return { ok: !1, reason: "missing-handoff-persisted", record: null };
  let inspect = options.inspectArtifactText;
  if (typeof inspect != "function")
    return { ok: !1, reason: "inspect-unavailable", record: null };
  let inspected = inspect(rawArtifact, options.pluginRoot), fields = inspected.artifact?.fields;
  if (inspected.errors.length > 0 || !fields) return { ok: !1, reason: "invalid-artifact", record: null };
  if ((inspected.normalizations ?? []).length > 0) return { ok: !1, reason: "normalized-artifact", record: null };
  if (fields.artifact !== "delivery-evidence" || fields.schema !== 5)
    return { ok: !1, reason: "not-schema5-delivery-evidence", record: null };
  if (fields.id !== id) return { ok: !1, reason: "artifact-id-mismatch", record: null };
  if (!["complete", "provisional", "blocked"].includes(fields.status))
    return { ok: !1, reason: "invalid-status", record: null };
  if (fields.status !== structured.status) return { ok: !1, reason: "status-mismatch", record: null };
  if (fields.root_plan_id !== structured.root_plan_id)
    return { ok: !1, reason: "root-plan-id-mismatch", record: null };
  if ((fields.subject_id ?? null) !== (structured.subject_id ?? null))
    return { ok: !1, reason: "structured-subject-mismatch", record: null };
  if ((fields.source_review_id ?? null) !== (structured.source_review_id ?? null))
    return { ok: !1, reason: "structured-source-review-mismatch", record: null };
  if ((fields.predecessor_evidence_id ?? null) !== (structured.predecessor_evidence_id ?? null))
    return { ok: !1, reason: "structured-predecessor-mismatch", record: null };
  let activeRoot = options.activeRootPlanId ?? null, closeoutRoot = options.closeoutRootPlanId ?? null;
  if (activeRoot && fields.root_plan_id !== activeRoot)
    return { ok: !1, reason: "active-root-mismatch", record: null };
  if (closeoutRoot && fields.root_plan_id !== closeoutRoot)
    return { ok: !1, reason: "closeout-root-mismatch", record: null };
  let derivedHash = sha256RawUtf8(rawArtifact);
  if (structured.artifact_hash !== derivedHash)
    return { ok: !1, reason: "artifact-hash-mismatch", record: null };
  let expected = options.expectedLineage;
  if (expected == null || typeof expected != "object" || Array.isArray(expected))
    return { ok: !1, reason: "missing-expected-lineage", record: null };
  if (expected.root_plan_id && fields.root_plan_id !== expected.root_plan_id)
    return { ok: !1, reason: "lineage-root-mismatch", record: null };
  if ((fields.subject_id ?? null) !== (expected.subject_id ?? null))
    return { ok: !1, reason: "lineage-subject-mismatch", record: null };
  if ((fields.source_review_id ?? null) !== (expected.source_review_id ?? null))
    return { ok: !1, reason: "lineage-source-review-mismatch", record: null };
  if (Object.prototype.hasOwnProperty.call(expected, "predecessor_evidence_id") && (fields.predecessor_evidence_id ?? null) !== (expected.predecessor_evidence_id ?? null))
    return { ok: !1, reason: "lineage-predecessor-mismatch", record: null };
  let structuredRootHash = typeof structured.root_content_hash == "string" ? structured.root_content_hash : null;
  if (!structuredRootHash || !/^[a-f0-9]{64}$/.test(structuredRootHash))
    return { ok: !1, reason: "missing-root-content-hash", record: null };
  let activeRootHash = typeof options.activeRootContentHash == "string" ? options.activeRootContentHash : null;
  return !activeRootHash || !/^[a-f0-9]{64}$/.test(activeRootHash) ? { ok: !1, reason: "missing-active-root-content-hash", record: null } : structuredRootHash !== activeRootHash ? { ok: !1, reason: "root-content-hash-mismatch", record: null } : {
    ok: !0,
    reason: null,
    record: {
      id,
...
function lineBoundedArtifactIndexes(message, exactArtifact) {
  let prose = String(message ?? ""), exact = String(exactArtifact ?? "");
  if (!exact) return [];
  let indexes = [], searchFrom = 0;
  for (; searchFrom <= prose.length; ) {
    let index = prose.indexOf(exact, searchFrom);
    if (index < 0) break;
    let beforeOk = index === 0 || prose[index - 1] === `
`, afterIndex = index + exact.length, afterOk = afterIndex === prose.length || prose[afterIndex] === `
`;
    beforeOk && afterOk && indexes.push(index), searchFrom = index + 1;
  }
  return indexes;
}
function boundedEvidenceAttachmentCount(message, expectedArtifact) {
  if (!expectedArtifact) return 0;
  let exact = String(expectedArtifact);
  if (!exact) return 0;
  let raw = String(message ?? ""), lines = raw.split(/\r?\n/), count = 0;
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!closed) continue;
    let joiner = exact.includes(`\r
`) ? `\r
` : `
`, inner = `${body.join(joiner)}${body.length > 0 ? joiner : ""}`;
    (inner === exact || inner === exact.replace(/\r?\n$/, "") || `${inner}${joiner}` === exact || `${inner}
` === exact) && (count += 1), index = cursor;
  }
  return count + lineBoundedArtifactIndexes(raw, exact).length;
}
function parseDeliveryReport(text) {
  let attestations = extractWorkflowAttestations(text).map((entry) => entry.value).filter((value) => asObject(value) && value.schema === 1 && value.kind === DELIVERY_REPORT_KIND);
  if (attestations.length !== 1)
    return { ok: !1, reason: "delivery-report-count", report: null };
  let report = attestations[0], id = typeof report.delivery_evidence_id == "string" ? report.delivery_evidence_id : "";
  if (!EVIDENCE_ID.test(id)) return { ok: !1, reason: "invalid-delivery-evidence-id", report: null };
  let keys = Object.keys(report).sort(), allowed = ["delivery_evidence_id", "kind", "schema"];
  return keys.some((key) => !allowed.includes(key)) ? { ok: !1, reason: "unexpected-delivery-report-fields", report: null } : { ok: !0, reason: null, report: { delivery_evidence_id: id } };
}
function rawArtifactOccurrenceCount(message, expectedArtifact) {
  let prose = String(message ?? ""), exact = String(expectedArtifact ?? "");
  if (!exact) return 0;
  let count = 0, searchFrom = 0;
  for (; searchFrom <= prose.length; ) {
    let index = prose.indexOf(exact, searchFrom);
    if (index < 0) break;
    count += 1, searchFrom = index + exact.length;
  }
  return count;
}
function evaluateDeliveryCompletion(message, turn) {
  if (!turn?.closeout_recorded || !turn.delivery_evidence_id || !turn.delivery_evidence_artifact || typeof turn.handoff_persisted != "boolean")
    return { ok: !1, reason: "closeout-not-recorded" };
  if (turn.active_root_plan_id && turn.delivery_evidence_root_plan_id && turn.active_root_plan_id !== turn.delivery_evidence_root_plan_id)
    return { ok: !1, reason: "active-root-mismatch" };
  let report = parseDeliveryReport(message);
  if (!report.ok) return { ok: !1, reason: report.reason };
  if (report.report.delivery_evidence_id !== turn.delivery_evidence_id)
    return { ok: !1, reason: "delivery-report-id-mismatch" };
  let attachmentCount = boundedEvidenceAttachmentCount(message, turn.delivery_evidence_artifact), occurrenceCount = rawArtifactOccurrenceCount(message, turn.delivery_evidence_artifact);
  if (turn.handoff_persisted === !1) {
    if (attachmentCount !== 1 || occurrenceCount !== 1)
      return { ok: !1, reason: "unpersisted-attachment-required" };
  } else if (attachmentCount !== 0 || occurrenceCount !== 0)
    return { ok: !1, reason: "persisted-attachment-invalid" };
  return { ok: !0, reason: null, delivery_evidence_id: turn.delivery_evidence_id };
}
...
function formatDeliveryReportFence(deliveryEvidenceId) {
  return [
    "```yaml workflow-attestation",
    "schema: 1",
...
function sharedArtifactStateRoot(workspaceRoot, options = {}) {
  let base = options.baseRoot ?? process.env.GELDMACHER_WORKFLOW_SHARED_ROOT ?? join3(sharedWorkflowHome(options), "state");
...
var slicePattern = /\bSLICE-[1-9][0-9]*\b/g, learningPattern = /\bLRN-[A-Za-z0-9][A-Za-z0-9-]*\b/g, modelInheritMarker = "[workflow-model-inherit-v1]", requiredScopeCategories = ["required", "permitted", "prohibited"], baselineKinds = ["repository", "head", "dirty-files", "known-failures", "targets-and-prerequisites"], sectionAliases = Object.freeze({
...
function fingerprintMap(value) {
  let entries = [...String(value ?? "").matchAll(/`([^`]+)`=([a-f0-9]{64})/g)];
...
function planData(artifact) {
  if (artifact.fields.schema >= 4) {
    ...
    })), evidenceClasses = declaredWithClass.length > 0 ? new Map(declaredWithClass.map((row) => [row["Check ID"], row["Evidence Class"]])) : new Map(checks2.map((row) => [row["Check ID"], "human-review-required"])), objectiveIds = objectives2.map((row) => row["Objective ID"]);
    return {
...
function validatePlanV4(parsed, sections, failures) {
  ...
  if (verification.length === 0 && parsed.normalizations.push("synthesized strategy checks from acceptance outcomes"), data.objectives.size !== parsed.fields.acceptance.length && failures.push("acceptance outcomes must map one-to-one to objectives"), parsed.wrapper) {
    let todos = parsed.wrapper.todos ?? [], finalTodo = todos.at(-1) ?? null, final = String(finalTodo?.content ?? "");
    if (!(todos.some((todo) => String(todo.content ?? "").includes(modelInheritMarker)) || String(parsed.wrapper.overview ?? "").includes(modelInheritMarker) || String(parsed.wrapper.name ?? "").includes(modelInheritMarker)) && !final.startsWith(modelInheritMarker) && failures.push(`native Plan must include ${modelInheritMarker} on the final closeout todo or plan overview`), todos.length > 0 && !final.startsWith(modelInheritMarker) && !todos.some((todo) => String(todo.content ?? "").startsWith(modelInheritMarker)) && failures.push(`final native todo must start with ${modelInheritMarker} when no other todo carries it`), /verify|check|evidence|snapshot|close\s*out/i.test(final) || failures.push("final native todo must verify or evidence the implemented result"), parsed.fields.schema === 5) {
      final.startsWith(modelInheritMarker) || failures.push(`final native todo must start with ${modelInheritMarker}`);
      for (let issue2 of planCloseoutAttestationIssues(finalTodo ?? final, { role: "final native todo" }))
...
function validateArtifactText(text, root = defaultRoot, options = {}) {
...
function validateArtifactSet(entries, root = defaultRoot, options = {}) {
...
process.argv[1] && ["validate-artifact.source.mjs", "validate-artifact.mjs"].includes(basename(process.argv[1])) && realpathSync(resolve4(process.argv[1])) === realpathSync(fileURLToPath(import.meta.url)) && runCli();
...
function repositoryPathFingerprint(workspaceRoot, repositoryPath) {
  let absolute = resolve5(workspaceRoot, repositoryPath);
  try {
    let stat = lstatSync(absolute);
    return stat.isSymbolicLink() ? `symlink:${sha2562(Buffer.from(readlinkSync(absolute), "utf8"))}` : stat.isFile() ? `file:${stat.mode.toString(8)}:${sha2562(readFileSync3(absolute))}` : stat.isDirectory() ? `directory:${stat.mode.toString(8)}` : `other:${stat.mode.toString(8)}:${stat.size}`;
  } catch (error) {
...
function captureRepositorySnapshot(workspaceRoot, options = {}) {
  let root = String(git(workspaceRoot, ["rev-parse", "--show-toplevel"], options)).trim(), head = String(git(root, ["rev-parse", "HEAD"], options)).trim(), index = git(root, ["ls-files", "--stage", "-z", "--"], options), status = git(root, ["status", "--porcelain=v2", "--untracked-files=all", "-z", "--"], options), tracked = nulPaths(git(root, ["diff", "--name-only", "-z", "HEAD", "--"], options)), untracked = nulPaths(git(root, ["ls-files", "--others", "--exclude-standard", "-z", "--"], options)), dirtyPaths = [.../* @__PURE__ */ new Set([...tracked, ...untracked])].sort(), fingerprints = Object.fromEntries(dirtyPaths.map((path) => [path, repositoryPathFingerprint(root, path)]));
  return {
    schema: 1,
...
function deriveRepositoryDelta(baseline, current) {
  ...
  let changedPaths = [.../* @__PURE__ */ new Set([...baseline.dirty_paths, ...current.dirty_paths])].sort().filter((path) => {
...
function evidenceRepositorySnapshot(snapshot, relevantPaths, { baselineAvailable = !0 } = {}) {
  if (!validSnapshot(snapshot)) throw new Error("repository snapshot is invalid");
  let entries = [...new Set(relevantPaths ?? [])].sort().map((path) => `${path}=${snapshot.fingerprints[path] ?? repositoryPathFingerprint(snapshot.repository_root, path)}`);
  return entries.push(`index=${snapshot.index_fingerprint ?? "unavailable"}`), entries.push(`status=${snapshot.status_fingerprint ?? "unavailable"}`), {
...
var sha2563 = manualReceiptHash, stableJson = stableManualReceiptJson;
...
function readOnlyShellSegment(segment) {
  let cleaned = String(segment ?? "").trim().replace(/^(?:[A-Za-z_][A-Za-z0-9_]*=[^\s]+\s+)*/, "").replace(/^rtk\s+/, "");
...
function isReadOnlyShell(command) {
  ...
  return !source.trim() || /[\r\n`]|\$\(|<|(?:^|[^<])>(?:>|&)?|(?:^|[^&])&(?!&)|\btee\b|\bsed\s+-i\b|\bperl\s+-i\b/.test(source) ? !1 : source.split(/\s*(?:&&|\|\||;|\|)\s*/).filter(Boolean).every(readOnlyShellSegment);
}
function normalizedWorkingDirectory(workspaceRoot, value) {
  let root = resolve6(workspaceRoot), source = value == null || String(value).trim() === "" ? root : resolve6(root, String(value)), rel = relative(root, source).replaceAll("\\", "/");
...
function shellToolInput(toolName, toolInput, workspaceRoot) {
  ...
  let source = toolInput && typeof toolInput == "object" && !Array.isArray(toolInput) ? toolInput : {}, command = source.command ?? source.cmd;
  if (typeof command != "string" || !command.trim()) return null;
  let workingDirectory = normalizedWorkingDirectory(workspaceRoot, source.workdir ?? source.cwd);
...
function beginManualCheckReceipt({
  ...
  let invocation = shellToolInput(toolName, toolInput, workspaceRoot);
  if (!invocation) return null;
  let plan = manualMachineChecks(rootPlanText, pluginRoot), matches = plan.checks.filter((check) => check.command === invocation.command && check.working_directory !== null && check.working_directory === invocation.working_directory);
  if (matches.length !== 1) return null;
  let snapshot = captureSnapshot(workspaceRoot), startedAt = now().toISOString();
  return {
    schema: 1,
...
`);
  if (!value || typeof value != "object" || seen.has(value)) return null;
  seen.add(value);
  for (let key of ["stdout", "output", "text"])
    if (typeof value[key] == "string") return value[key].replace(/\r\n/g, `
`);
  if (Array.isArray(value.content)) {
    let text = value.content.filter((entry) => entry?.type === "text" && typeof entry.text == "string").map((entry) => entry.text).join(`
`);
    if (text) return text.replace(/\r\n/g, `
`);
  }
  for (let child of Object.values(value)) {
    let found = responseText(child, seen);
    if (found !== null) return found;
  }
  return null;
}
function explicitHostSignal(value, seen = /* @__PURE__ */ new Set()) {
  if (!value || typeof value != "object" || seen.has(value)) return null;
  if (seen.add(value), value.isError === !0 || Object.hasOwn(value, "error") && value.error !== null && value.error !== !1 && value.error !== "") return "failed";
  for (let key of ["status", "result_status", "outcome"]) {
    let signal = String(value[key] ?? "").trim().toLowerCase();
    if (["failed", "failure", "error", "cancelled", "canceled", "timed-out", "timeout"].includes(signal)) return "failed";
  }
  for (let key of ["ok", "success"]) if (value[key] === !1) return "failed";
  let success = !1;
  for (let key of ["status", "result_status", "outcome"]) {
    let signal = String(value[key] ?? "").trim().toLowerCase();
    ["passed", "success", "succeeded", "completed", "ok"].includes(signal) && (success = !0);
  }
  for (let key of ["ok", "success"]) value[key] === !0 && (success = !0);
  for (let child of Object.values(value)) {
    let nested = explicitHostSignal(child, seen);
    if (nested === "failed") return "failed";
    nested === "passed" && (success = !0);
  }
  return success ? "passed" : null;
}
function manualToolResultStatus(response) {
  if (response == null) return { status: "missing", exit_code: null };
  let signal = explicitHostSignal(response), textualExit = responseText(response)?.match(/\b(?:process\s+)?(?:exited?|exit)\s+(?:with\s+)?(?:code|status)\s*[:=]?\s*(-?\d+)\b/i), exitCode = explicitExitCode(response) ?? (textualExit ? Number(textualExit[1]) : null);
  return signal === "failed" || exitCode !== null && exitCode !== 0 ? { status: "failed", exit_code: exitCode } : exitCode === 0 || signal === "passed" ? { status: "passed", exit_code: exitCode } : { status: "missing", exit_code: null };
}
...
function assertManualReceiptPath(path, base) {
  let resolvedBase = resolve6(base), resolvedPath = resolve6(path);
  ...
  if (existsSync3(current) && lstatSync2(current).isSymbolicLink())
...
function ensureDirectory(path, base) {
  assertSafeDirectory(path, base), mkdirSync(path, { recursive: !0, mode: 448 });
  let current = resolve6(path), stop = resolve6(base);
...
function readManualReceiptRecord(path, base) {
  ...
  let value = JSON.parse(readFileSync4(path, "utf8"));
  return value && typeof value == "object" && !Array.isArray(value) ? value : null;
}
var readReceiptRecord = readManualReceiptRecord;
function existingRecords(directory, base) {
  return existsSync3(directory) ? (assertSafeDirectory(directory, base), lstatSync2(directory).isSymbolicLink() ? [] : readdirSync(directory).filter((name) => /^[a-f0-9]{64}\.json$/.test(name)).flatMap((name) => {
    try {
      let record = readReceiptRecord(join5(directory, name), base);
...
function invalidateManualCheckReceipts({ rootPlanText, workspaceRoot, options = {} }) {
  ...
  return rmSync(base, { recursive: !0, force: !0 }), !0;
}
function completeManualCheckReceipt({
  ...
  let current = captureSnapshot(workspaceRoot), currentFingerprint = repositorySnapshotFingerprint(current);
  if (resolve6(current.repository_root) !== resolve6(candidate.repository_root) || currentFingerprint !== candidate.snapshot_fingerprint)
    return invalidateManualCheckReceipts({ rootPlanText, workspaceRoot, options }), { status: "repository-mutated", receipt: null };
  let result = manualToolResultStatus(toolResponse);
  if (result.status === "missing") return { status: "missing-result", receipt: null };
  let canonicalRoot = current.repository_root, base = proofBase(canonicalRoot, candidate.root_hash, options), directory = join5(base, candidate.check_id), ordinal = existingRecords(directory, sharedArtifactStateRoot(canonicalRoot, options)).filter((record2) => record2?.receipt?.snapshot_fingerprint === candidate.snapshot_fingerprint).length + 1, receipt = {
    schema: 1,
  ...
  }, receiptHash = sha2563(stableJson(receipt)), recordedAt = now(), record = {
    schema: 1,
  ...
  return writeReceiptRecord(join5(directory, `${receiptHash}.json`), record, sharedArtifactStateRoot(canonicalRoot, options)), { status: result.status === "passed" ? "recorded" : "failed", receipt, receipt_hash: receiptHash };
}
function validStoredReceipt(record, { plan, repositoryRoot, currentFingerprint, now }) {
  ...
  if (!receipt || receipt.schema !== 1 || receipt.kind !== "manual-check-receipt" || !/^[a-f0-9]{64}$/.test(String(record.receipt_hash ?? "")) || sha2563(stableJson(receipt)) !== record.receipt_hash || receipt.root_hash !== plan.root_hash || receipt.repository_key !== repositoryKey(repositoryRoot) || receipt.snapshot_fingerprint !== currentFingerprint) return !1;
...
function loadManualCheckReceipts({
  ...
  try {
    return assertSafeDirectory(base, stateRoot), lstatSync2(base).isSymbolicLink() ? [] : readdirSync(base, { withFileTypes: !0 }).filter((entry) => entry.isDirectory() && /^CHECK-[1-9][0-9]*$/.test(entry.name)).flatMap((entry) => existingRecords(join5(base, entry.name), stateRoot)).filter((record) => validStoredReceipt(record, {
      plan,
...
function manualConstraintProjection({ checks = [], evidence = [], pending = !1 }) {
  let required = checks.filter((check) => check.Required === "yes"), byId = new Map((evidence ?? []).map((entry) => [entry.check_id, entry])), ids2 = (predicate) => required.filter(predicate).map((check) => check["Check ID"]), hostAttested = ids2((check) => {
  ...
  }), machine = ids2((check) => check["Evidence Class"] === "machine-verifiable"), failed = ids2((check) => byId.get(check["Check ID"])?.grade === "failed"), unattestedVerified = pending ? [] : ids2((check) => {
    let entry = byId.get(check["Check ID"]);
  ...
  }), ordinaryGaps = pending ? [] : ids2((check) => {
    let entry = byId.get(check["Check ID"]);
    return !entry || ["supported", "partial", "unavailable"].includes(entry.grade);
  }), gaps = unique2([...unattestedVerified, ...ordinaryGaps]), humanReview = ids2((check) => check["Evidence Class"] === "human-review-required"), humanApproval = ids2((check) => check["Evidence Class"] === "human-approval-required"), reasons = pending ? [] : [
...
import { dirname as dirname3, isAbsolute, relative as relative2, resolve as resolve7, sep as sep2 } from "node:path";
...
function repositoryAuthorityPaths(repositoryRoot, repositoryPath) {
  let root = realpathSync3(repositoryRoot), lexical = resolve7(root, repositoryPath);
  ...
  for (; !existsSync4(existing) && existing !== root; ) existing = dirname3(existing);
  let resolvedExisting = realpathSync3(existing);
  ...
  let unresolved = relative2(existing, lexical), resolved = resolve7(resolvedExisting, unresolved);
...
function patchTargets(value) {
  let source = String(value ?? ""), paths = [];
  for (let match of source.matchAll(/^\*\*\* (?:Update|Add|Delete) File:\s*(.+?)\s*$/gm)) paths.push(match[1]);
  for (let match of source.matchAll(/^\*\*\* Move to:\s*(.+?)\s*$/gm)) paths.push(match[1]);
  for (let match of source.matchAll(/^(?:\+\+\+|---)\s+(?:[ab]\/)?(.+?)\s*$/gm))
    match[1] !== "/dev/null" && paths.push(match[1]);
  for (let match of source.matchAll(/^(?:rename|copy) (?:from|to)\s+(.+?)\s*$/gm)) paths.push(match[1]);
  return paths;
}
function directInputPaths(value, key = null) {
  return typeof value == "string" ? [
    ...
    "destination_path",
    "notebook_path"
  ].includes(key) ? [value] : [] : Array.isArray(value) ? value.flatMap((entry) => directInputPaths(entry, key)) : !value || typeof value != "object" ? [] : Object.entries(value).flatMap(([entryKey, entry]) => directInputPaths(entry, entryKey));
}
function repositoryRelativeTarget(target, repositoryRoot) {
  let raw = String(target).trim();
  return raw ? raw.includes("\\") || raw.includes("\0") ? raw : relative2(resolve7(repositoryRoot), resolve7(repositoryRoot, raw)).replace(/\/$/, "") : null;
}
function directMutationTargets({ toolName, toolInput, repositoryRoot }) {
  ...
  if (typeof toolInput == "string")
    try {
      ...
      parsed && typeof parsed == "object" && !Array.isArray(parsed) && (input = parsed);
    } catch {
    }
  let patch = typeof toolInput == "string" ? toolInput : input.patch ?? input.diff ?? input.input ?? "", targets = uniqueSorted([
...
function readTipFile(path, rootPlanId) {
  return existsSync5(path) ? validateTip(JSON.parse(readFileSync5(path, "utf8")), rootPlanId) : null;
}
...
function atomicJson(path, value) {
  mkdirSync2(dirname4(path), { recursive: !0, mode: 448 });
...
`), descriptor;
    ...
    let stale = !1;
    try {
      stale = !processAlive(JSON.parse(readFileSync5(path, "utf8")).pid);
    } catch {
      stale = !0;
    }
...
function recordFor(text, pluginRoot) {
  let fields = parsedArtifact(text, pluginRoot).fields;
  return {
...
function validateRecord(record) {
  if (record?.handoff_record_schema !== HANDOFF_RECORD_SCHEMA || record?.artifact_schema !== 5 || record?.controller_protocol !== 5 || !/^(?:wp|de|wr)-[A-Za-z0-9][A-Za-z0-9-]*$/.test(String(record?.artifact_id ?? "")) || record?.text_hash !== sha2564(record?.text ?? ""))
...
var ArtifactHandoffStore = class {
  constructor(root, pluginRoot, artifactSetOptions = {}) {
    this.root = resolve8(root), this.pluginRoot = resolve8(pluginRoot), this.artifactSetOptions = artifactSetOptions ?? {}, this.directory = join6(this.root, "handoff", "artifacts");
  }
  ...
  loadIndex({ repair = !1 } = {}) {
    try {
      let index = JSON.parse(readFileSync5(this.indexPath(), "utf8")), actual = existsSync5(this.directory) ? readdirSync2(this.directory, { withFileTypes: !0 }).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => entry.name.slice(0, -5)).sort() : [], recorded = (index.entries ?? []).map((entry) => entry.artifact_id).sort();
...
`)) throw new Error("handoff index mismatch");
  ...
  records(ids2 = null) {
    return existsSync5(this.directory) ? (ids2 ? [...new Set(ids2)].map((id) => this.artifactPath(id)).filter(existsSync5) : readdirSync2(this.directory, { withFileTypes: !0 }).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => join6(this.directory, entry.name))).map((path) => validateRecord(JSON.parse(readFileSync5(path, "utf8")))).sort((left, right) => left.artifact_id.localeCompare(right.artifact_id)) : [];
  }
  record(artifacts) {
    ...
    let lockPath = join6(this.root, "handoff", ".lock"), descriptor;
    try {
  ...
  quarantineArtifact(artifactId, { expectedTextHash, apply = !1, now = () => /* @__PURE__ */ new Date() } = {}) {
    ...
    let dependents = this.loadIndex({ repair: !1 }).entries.filter((entry) => (entry.references ?? []).includes(artifactId)).map((entry) => entry.artifact_id).sort(), timestamp = now().toISOString().replace(/[:.]/g, "-"), quarantineDirectory = join6(this.root, "handoff", "quarantine", `${timestamp}-${artifactId}-${expectedTextHash.slice(0, 12)}`), source = this.artifactPath(artifactId), target = join6(quarantineDirectory, "artifact-record.json"), report = {
      command: "quarantine-handoff",
      namespace_root: this.root,
      artifact_id: artifactId,
      artifact_type: record.artifact_type,
      text_hash: record.text_hash,
      record_hash: createHash6("sha256").update(readFileSync5(source)).digest("hex"),
      source,
    ...
    try {
      ...
      mkdirSync2(quarantineDirectory, { recursive: !0, mode: 448 });
      let indexPath = this.indexPath();
      existsSync5(indexPath) && writeFileSync2(join6(quarantineDirectory, "index-before.json"), readFileSync5(indexPath), { mode: 384, flag: "wx" }), renameSync2(source, target);
      try {
        ...
        throw renameSync2(target, source), atomicJson(this.indexPath(), currentIndex), error;
      }
...
function unresolvedRootCheckMap(contract, predecessorEvidence) {
  let effectiveChecks = predecessorEvidence?.effective?.checks;
...
function normalizeCheckEvidence(input, plannedChecks, rootChecks, evidenceMode2, {
  ...
  let ids2 = input.map((entry) => entry?.check_id);
  if (new Set(ids2).size !== ids2.length) throw new Error("closeout Check evidence IDs must be unique");
  for (let id of plannedChecks.keys()) if (!ids2.includes(id)) throw new Error(`closeout is missing required Check ${id}`);
...
function buildDeliveryEvidence({
  ...
  if (evidenceTipId) {
    if (!review || review.fields.latest_evidence_id !== evidenceTipId || review.fields.next_action !== "correct" || !review.fields.correction_id || !review.correction) {
      ...
      if ((checkEvidence ?? []).length > 0 || changedPaths.length > 0) {
        let entries2 = normalizeCheckEvidence(checkEvidence, expectedCheckMap(contract, null), rootCheckMap(contract), mode, {
          enforceManualCheckReceipts: requireManualReceipts,
          manualCheckReceipts,
          existingCheckEvidence: existingFields?.check_evidence ?? []
        }), suppliedPaths = unique3(changedPaths.map(String).map((path) => path.trim()).filter(Boolean)).sort(), expectedSeed = evidenceSeed({
          contract,
        ...
        if (!(JSON.stringify(stable2(entries2)) === JSON.stringify(stable2(existingFields?.check_evidence ?? [])) && JSON.stringify(suppliedPaths) === JSON.stringify(existingFields?.changed_paths ?? []) && (mode === "lean" || (existingFields?.strategy_revision ?? 0) === effectiveStrategyRevision) && expectedId === evidenceTipId)) throw new Error(`stale or competing closeout conflicts with current Evidence tip ${evidenceTipId}`);
      }
  ...
  let roots = rootCheckMap(contract), unresolvedRootChecks = correction ? unresolvedRootCheckMap(contract, priorInspection.effective.get(evidenceTipId)) : /* @__PURE__ */ new Map(), suppliedCheckIds = new Set((checkEvidence ?? []).map((entry) => entry?.check_id)), missingRootRefresh = [...unresolvedRootChecks.keys()].filter((checkId) => !suppliedCheckIds.has(checkId));
  ...
  let plannedChecks = expectedCheckMap(contract, correction, unresolvedRootChecks), entries = normalizeCheckEvidence(checkEvidence, plannedChecks, roots, mode, {
    enforceManualCheckReceipts: requireManualReceipts,
    manualCheckReceipts
  }), grade = overallGrade(entries), status = artifactStatus(grade), rootObjectives = contract.objectives, affectedObjectives = correction ? unique3([...correctionObjectives(correction), ...entries.flatMap((entry) => checkObjectives(roots.get(entry.check_id)))]) : [...rootObjectives], affected = affectedObjectives.length > 0 ? affectedObjectives : [...rootObjectives], reusedObjectives = representation === "delta" ? rootObjectives.filter((id2) => !affected.includes(id2)) : [], executedChecks = entries.map((entry) => entry.check_id), reusedChecks = representation === "delta" ? [...roots.keys()].filter((id2) => !executedChecks.includes(id2)) : [], paths = unique3(changedPaths.map(String).map((path) => path.trim()).filter(Boolean)).sort(), seed = evidenceSeed({
    contract,
    subjectId,
    sourceReviewId,
    predecessorEvidenceId,
...
`, finalEntries = [...normalized.entries, { label: id, text: artifact }], inspection = inspectArtifactSet(finalEntries.map((entry) => [entry.label, entry.text]), pluginRoot);
  ...
  let projection = manualConstraintProjection({ checks: contract.checks, evidence: entries });
  return {
    duplicate: !1,
    artifact,
...
function provisionalWithoutBaseline(report) {
  let limitation = "No pre-mutation repository baseline was available; changed paths are the unambiguous current dirty set and otherwise verified observations are capped at supported.";
  return {
    ...report,
...
function assertCorrectionSourceReview(rootPlanText, rootFields, chain, pluginRoot) {
  let entries = [{ label: rootFields.id, text: rootPlanText }, ...chain], inspected = inspectArtifactSet(entries.map((entry) => [entry.label, entry.text]), pluginRoot);
  if (inspected.errors.length > 0)
    throw new Error(`native correction closeout chain is invalid: ${inspected.errors.join("; ")}`);
  let tips = effectiveCliSummary(inspected), evidenceTip = tips.evidence_tips[rootFields.id] ?? null;
  if (!evidenceTip) throw new Error("native correction closeout is missing exact predecessor Evidence");
  let reviewTip = tips.review_tips[rootFields.id] ?? null, review = reviewTip ? inspected.effective.get(reviewTip) : null;
...
function performNativeCloseout({
  ...
  let parsed = attestation ? validateCloseoutInput(attestation) : parseCloseoutInput(message, { expectedPhase });
  ...
  let inspectedRoot = inspectArtifactText(rootPlanText, pluginRoot);
  ...
  let rootFields = inspectedRoot.artifact.fields;
  ...
  if (assertChangedPathAuthority(rootFields, repositoryDelta.changed_paths, repositoryDelta.repository_snapshot.repository_root ?? repositoryDelta.repository_root), !repositoryDelta.baseline_available && report.phase !== "review-recovery")
    throw new Error("native implementation and correction closeout require a pre-mutation repository baseline");
  let handoffStore = createContentAddressedHandoffStore(rootPlanText, pluginRoot, handoffOptions), context = handoffStore.context(rootFields.id, rootPlanText), cached = context.artifacts, cachedEvidenceTip = context.evidence_tip, effectiveReport = repositoryDelta.baseline_available || cachedEvidenceTip ? report : provisionalWithoutBaseline(report), chain = mergeArtifacts([
    ...cached,
    ...artifacts
  ], pluginRoot).filter((entry) => entry.label !== rootFields.id);
  effectiveReport.phase === "correction" && assertCorrectionSourceReview(rootPlanText, rootFields, chain, pluginRoot);
  let repositoryRoot = repositoryDelta.repository_snapshot.repository_root ?? repositoryDelta.repository_root, manualCheckReceipts = loadManualCheckReceipts({
    rootPlanText,
    pluginRoot,
    workspaceRoot: repositoryRoot,
    options: receiptOptions
  }), closeout = buildDeliveryEvidence({
    rootPlanText,
    artifacts: chain,
  ...
  return invalidateManualCheckReceipts({ rootPlanText, workspaceRoot: repositoryRoot, options: receiptOptions }), rememberContentAddressedRoot(rootPlanText, pluginRoot, handoffOptions), {
    ...persisted,
...
function normalizeManualPrimaryAction(presentation, action) {
  return !["blocked", "failed"].includes(presentation?.outcome) || SAFE_BLOCKED_ACTIONS.has(action) ? action : action === "implement-plan" ? "repair-root" : ["accept-provisional", "approve-correction", "correct"].includes(action) ? "retry-review" : "provide-artifacts";
}
function deriveManualJourneyState(presentation, action) {
  let state = presentation?.workflow_state;
  return ["achieved", "accepted-provisional"].includes(state) || action === "none" ? "done" : action === "implement-plan" ? "plan-ready" : action === "approve-correction" ? "correction-approval-required" : action === "replan" ? "replan-approval-required" : action === "accept-provisional" ? "provisional-acceptance-required" : ["blocked", "failed"].includes(presentation?.outcome) ? "blocked" : ["review-root", "retry-review"].includes(action) ? "review-ready" : ["answer", "resolve-intent", "provide-artifacts"].includes(action) ? "clarification-required" : ["closeout", "attach-artifact"].includes(action) ? "closeout-recovery-required" : presentation?.phase === "review" ? "review-active" : "implementation-active";
}
function taskBoundManualInvoke(action, trace = {}) {
  let catalog = MANUAL_PRIMARY_ACTIONS[action] ?? { command: String(action), label: String(action) }, root = trace.root_plan_id ?? null, evidence = trace.evidence_id ?? null, review = trace.review_id ?? null;
  return action === "none" ? "No further Workflow action required" : action === "implement-plan" ? catalog.command : action === "accept-provisional" ? [catalog.command, root, "provisional"].filter(Boolean).join(" ") : action === "attach-artifact" ? [catalog.command, evidence ?? root].filter(Boolean).join(" ") : action === "answer" ? [catalog.command, review ?? root].filter(Boolean).join(" ") : [catalog.command, root].filter(Boolean).join(" ");
}
function manualJourneyDecision({ state = "blocked", blocker, action, trace = {} }) {
  let normalizedAction = normalizeManualPrimaryAction({ outcome: state === "blocked" ? "blocked" : "partial" }, action), label = MANUAL_JOURNEY_STATE_LABELS[state] ?? state, catalog = MANUAL_PRIMARY_ACTIONS[normalizedAction] ?? { label: normalizedAction };
...
import { join as join7, relative as relative3, resolve as resolve9 } from "node:path";
...
}), sha2566 = manualReceiptHash, stableJson2 = stableManualReceiptJson, canonicalWorkspaceRoot2 = canonicalManualWorkspaceRoot;
function normalizedObservedPaths(paths, repositoryRoot) {
  ...
  return [...new Set((paths ?? []).map((value) => {
    ...
    let candidate = resolve9(root, source), rel = relative3(root, candidate).replaceAll("\\", "/");
...
function receiptBase(workspaceRoot, rootHash, options = {}) {
  return join7(sharedArtifactStateRoot(workspaceRoot, options), "manual-boundary-receipts", rootHash);
}
function exactRoot(rootPlanText, pluginRoot) {
  let inspected = inspectArtifactText(rootPlanText, pluginRoot);
...
function receiptIdentity(receipt) {
  return `br-${sha2566(stableJson2({ ...receipt, receipt_id: void 0 }))}`;
}
function createManualBoundaryReceipt({
  ...
  let reason = MANUAL_BOUNDARY_RECOVERY_REASONS[recoveryErrorCode];
  ...
  let snapshot = captureSnapshot(workspaceRoot), repositoryRoot = canonicalWorkspaceRoot2(snapshot.repository_root);
  ...
  let paths = normalizedObservedPaths(observedPaths ?? snapshot.dirty_paths, repositoryRoot), currentPaths = normalizedObservedPaths(snapshot.dirty_paths, repositoryRoot);
  ...
  let record = {
    ...
    receipt_hash: sha2566(stableJson2(receipt)),
    receipt
  }, stateRoot = sharedArtifactStateRoot(repositoryRoot, options), path = join7(receiptBase(repositoryRoot, receipt.root_content_hash, options), `${receipt.receipt_id}.json`);
  return writeManualReceiptRecord(path, record, stateRoot), Object.freeze({ ...receipt, reason_codes: Object.freeze([...receipt.reason_codes]), observed_paths: Object.freeze([...receipt.observed_paths]) });
}
function verifyManualBoundaryReceipt({
  ...
  try {
    if (exactRoot(rootPlanText, pluginRoot), !receipt || typeof receipt != "object" || Array.isArray(receipt)) throw new Error("boundary receipt is missing");
    ...
    let stateRoot = sharedArtifactStateRoot(repositoryRoot, options), path = join7(receiptBase(repositoryRoot, receipt.root_content_hash, options), `${receipt.receipt_id}.json`), record = readManualReceiptRecord(path, stateRoot);
    ...
    if (record.repository_root !== repositoryRoot || record.receipt_hash !== sha2566(stableJson2(receipt)) || stableJson2(record.receipt) !== stableJson2(receipt))
      throw new Error("boundary receipt host record does not match the artifact");
    let observed = Date.parse(receipt.observed_at), expires = Date.parse(record.expires_at), currentTime = now().getTime();
