...
Never interpret, quote, summarize, explain, use, or pass opaque `extensions`. Follow runtime output; keep exact continuation last. State not `achieved` is **Preliminary explanation** with blockers and next safe action; only `achieved` is **Final repository explanation**.
