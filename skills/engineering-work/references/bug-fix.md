...
Use for one reported defect whose symptom must be reproduced, explained, corrected, and checked again. Apply the steps appropriate to the commissioned phase. Repository changes require an applicable plan and an implementation or correction instruction; this method does not commission the next Workflow phase.
...
8. Inspect the final diff for unsupported changes and leftover instrumentation. Self-verification completes the commissioned implementation or correction; a fresh Review is separately commissioned in manual work or delegated within an expressly commissioned Auto-Work sequence.
