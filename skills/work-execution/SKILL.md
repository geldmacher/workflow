...
After the implementation is stable, run the relevant Checks and capture the resulting snapshot. Return evidence in the response; do not create repository files merely to satisfy the Workflow protocol and do not materialize correction learning candidates. Complete evidence makes those candidates eligible for a later explicit `/learn-from-work`; it does not authorize guidance edits now. Lean/standard normally emits delta evidence; deep/uncertain work may emit full evidence. Include fresh/reused coverage, validate the effective chain when possible, and direct Ask `/review-work`. Never merge, push, create a PR, deploy, access production, or claim production success.
