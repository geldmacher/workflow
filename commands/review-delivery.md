...
6. Before emitting that improvement plan, use Cursor's interview tool (`AskQuestion`) when execution-critical clarification is needed; if it cannot be clarified, emit no plan and list the needed answers.
7. Treat that `Recommended next handoff` as the preferred input for the next `/execute-handoff` loop.
