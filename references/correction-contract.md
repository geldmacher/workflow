...
`/correct-work` resolves current tips, classifies `satisfied|pending|partial|conflicted`, and executes pending/partial FIXes. Closeout refreshes correction plus failed, missing, affected, stale, or ambiguous Root Checks. Unaffected proof keeps its grade. Equivalent Checks share one stable probe with honest Evidence per ID. Conflict, drift, expansion, or ambiguous approval blocks.
