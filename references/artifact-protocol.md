Schema 5 supports `work-plan|delivery-evidence|work-review`. Workflow-3/4 is immutable status-only history. The `wp-*` Intent Root is authoritative; Strategy and Decision Ledger evolve only within it.
...
`achieved` requires every Check verified. `accepted-provisional` is explicit controller acceptance or an ephemeral Manual snapshot with missing/unavailable proof and never qualifies. Failure is `blocked`; review cannot upgrade it.
