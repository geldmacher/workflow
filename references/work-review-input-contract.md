...
Repository assessment and proof calibration are separate. `supported` requires an unambiguous outcome on the exact current snapshot. Command invocation, source presence, masked exit status, or an unknown outcome is `partial` or `unavailable`. Exact observations already present in the same task may be reused while the repository snapshot is unchanged; protected attestation is not required for a provisional Manual decision.
