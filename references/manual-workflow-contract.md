...
Artifact text remains host-neutral and immutable. Every material Plan, implementation handoff, Review, status, explanation, acceptance, and controller delivery follows the [human-first output contract](./human-output-contract.md): `Quick decision`, a complete human `Details` deep dive, then the authoritative `Agent and machine contract`. The human layers add no authority and omit no decision-relevant meaning; exact machine artifacts stay complete and last. Actionable Manual-phase replies use the `### Next step` footer from [manual MCP output](./manual-mcp-output-contract.md) (Now/How/Why; Off track when blocked). Terminal `achieved` and `accepted-provisional` status use compact state-specific completion blocks. Prefer MCP `presentation` when present; keep command tokens exact.
