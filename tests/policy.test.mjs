...
test("planning uses compact semantic Root with immutable intent and adaptive strategy", () => {
  ...
  assert.match(runtime, /shell Checks inspect wrappers and pre\/post hooks;.*one classifiable command.*canonical repository directory/is);
  assert.match(runtime, /Review must not write repository content/i);
...
test("runtime surface has one bundled controller and no automatic publication", () => {
  ...
  assert.deepEqual(Object.values(hookConfig.hooks).map((entries) => entries.length), [1, 2, 1, 2, 1, 1, 2, 1]);
  for (const event of ["sessionStart", "beforeSubmitPrompt", "stop", "preToolUse", "subagentStart", "subagentStop", "postToolUse", "postToolUseFailure"]) assert.match(hooks, new RegExp(event));
  assert.doesNotMatch(hooks, /afterAgentResponse/);
  assert.equal(hookConfig.hooks.stop.length, 1);
  assert.equal(hookConfig.hooks.stop[0].failClosed, false);
  assert.doesNotMatch(hookConfig.hooks.stop[0].command, /--enforce/);
