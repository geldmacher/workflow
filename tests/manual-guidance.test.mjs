...
test("skills preserve fresh observation, human authority, and read-only Review", () => {
  ...
  assert.match(read("skills/work-planning/SKILL.md"), /shell Checks inspect wrappers and pre\/post hooks;.*one classifiable command.*canonical repository directory/is);
  assert.match(read("skills/work-planning/SKILL.md"), /Review must not write repository content/i);
  ...
  assert.match(read("skills/work-review/SKILL.md"), /observed.*exactly once.*Delivery Evidence and Work Review together or neither/is);
