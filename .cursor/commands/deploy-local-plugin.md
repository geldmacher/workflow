...
Do not add an `--all` mode, manually copy bundles, edit the personal Marketplace, delete Codex caches or restart either host. Report the selected hosts, installed local versions, receipts, applicable Codex cache verification, dirty provenance, and remaining host activation steps. After a real deployment, require a Cursor reload and/or a new Codex task for the hosts that changed before claiming live activation.
