...
Cursor and Codex keep ordinary prompts and tools available even when a Workflow hook, state file, or runtime dependency is unavailable. A healthy active Review denies mutation and permits only an exact approved Root Check in its canonical Git repository. If the guard itself is unavailable, the host remains usable and Workflow cannot claim verified delivery from that Review. A strictly reconstructed final `/review-work` transcript command can recover Review activation only provisionally; it never recovers Root authority, and the output explains Hook Trust/reload recovery.
