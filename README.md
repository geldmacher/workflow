...
Optional shared Manual preference: `~/.geldmacher/workflow/preferences.yaml` with `tool_approval: strict|allowlisted` steers whether you expect per-call host MCP confirmations or a host allowlist. It never grants approvals and never writes host settings. The same file may set `manual_subagent_policy` for concrete host-specific Manual subagent candidates; missing or invalid policy stays parent-only. Workflow MCP tools expose standard safety annotations; prefer least-privilege host presets (read-only, five Manual tools, or explicit full Cursor controller). Cursor allowlisted setup uses `permissions.json` with server key `workflow` and the plugin-prefixed chip id when shown; Codex uses Manual server key `geldmacher-workflow`. See [host approval](references/host-approval-contract.md) and [manual subagent policy](references/manual-subagent-policy.md).
...
The Codex Manual sequence is `$plan-work` in Plan mode, the separate human **Implement Plan** action, automatic `workflow_closeout`, a fresh `$review-work` task, then `$work-status`. Planning returns a native `<proposed_plan>` containing the visible `wp-*` ID. Hook policy blocks the wrong planning mode, review mutations, missing preflight, and missing closeout. With no Manual subagent policy it also blocks explicit subagent model overrides. When a Codex ordered policy is configured, Workflow may rewrite Agent inputs to the selected candidate and advance after unavailable-model failures; out-of-policy observed models remain invalid evidence. `SubagentStart` mismatches are marked invalid because that event cannot stop startup; later tools are blocked and the result cannot serve as evidence.

## Artifact protocol

New work uses Artifact Schema 5. The compact `work-plan` stores the immutable Intent Root and closed authority envelope. Replans form a linear lineage through paired `predecessor_plan_id` and `replan_source_review_id` fields; the lineage tip is the active Root. One deterministic builder derives Evidence identity, Intent hash, topology, mode, grade, and status from the validated chain and structured observations. Manual low/medium-risk work without Hard Triggers uses Lean Evidence; all higher-risk or controller work uses Full Evidence. An external Schema-1 root-content handoff cache, namespaced by the full SHA-256 of exact Root text, transports exact Root, Evidence, and Review text between contexts and hosts but creates no authority, Run, approval, acceptance, qualification, or Learning. Cursor MCP binds operational workspace identity through `GELDMACHER_WORKFLOW_WORKSPACE_ROOT=${workspaceFolder}` and keeps MCP `roots/list` as a compatibility fallback. `npm run migrate:handoff -- --workspace <root>` imports only verified existing repository-key Cursor Handoff records into content-addressed namespaces, leaves the source untouched, and writes a hash-complete migration receipt. Workflow-3/4 documents and Runs stay readable through status/watch but are not mutable or automatically converted. See [artifact protocol](references/artifact-protocol.md) and [Workflow-5 migration](docs/migration-workflow-5.md).
